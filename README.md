...
| Recruiting and rosters | `/football/recruiting/` | Search source-listed 2025–26 rosters, recruiting commitments, team talent and returning production with stable IDs, raw fields and release receipts |
| NCAA leaders | `/football/ncaa-leaders/` | Source-native NCAA player/game leaderboards for passing, rushing, receiving, defense, kicking and punt returns, with season archive pages from 2020 onward |
...
| Schedule clocks | `/api/basketball/research/schedule-times?season=2027&meta=1` | Exact-ID ESPN scoreboard observations showing which upcoming tips are source-confirmed versus date-only/TBD |
...
| ESPN prospect dossiers | `/basketball/recruiting/prospect/?season=…&id=…` | Exact-ID source-attributed recruiting rank, grade, position ranks, commitment, school and hometown context |
...
| NCAA game context | `/basketball/game-context/` | Inspect retained ESPN game-day roster status (starters, active and DNP flags) and officiating assignments across 2003–04 through 2025–26, with source receipts |
...
| National prospect board | `/basketball/recruiting/` | Search the live ESPN 2025–30 men's basketball classes by rank, grade, position, commitment and destination, with exact source IDs and prospect-card links |
...
The scheduled basketball job uses an incremental D1 maintenance export: it refreshes the newest source seasons and preserves the complete historical archive already stored in Cloudflare. GitHub Actions also caches the attributed release bytes, so historical files can be reused without another upstream request. This avoids replaying millions of unchanged rows and keeps the daily job inside D1 storage limits. The workflow’s `full_d1_rebuild` dispatch option, or a local run with `BASKETBALL_D1_INCREMENTAL` unset, is reserved for an empty-database bootstrap or deliberate historical repair.
