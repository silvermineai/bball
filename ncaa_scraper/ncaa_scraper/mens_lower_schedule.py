"""Contracts for the public NCAA men's DII/DIII basketball schedule API.
...
SCOREBOARD_URL = "https://www.ncaa.com/scoreboard/basketball-men/{division}"
SPORT_CODE = "MBB"
GENDER = "men"
...
def query_url(meta: str, query_hash: str, variables: dict[str, Any]) -> str:
    ...
    if variables.get("sportCode") != SPORT_CODE:
        raise ValueError("men's lower schedule queries must carry sportCode=MBB")
    division = variables.get("division")
    if division not in (2, 3):
        raise ValueError("men's lower schedule queries must carry division=2 or 3")
...
def parse_contests(payload: dict[str, Any], division: int, season_year: int, requested_date: str) -> list[dict[str, Any]]:
    ...
    for contest in contests:
        ...
        seen.add(key)
        if contest.get("sportUrl") != "basketball-men":
            raise ValueError("NCAA contest is outside men's basketball scope")
