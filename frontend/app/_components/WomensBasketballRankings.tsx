  ...
  return <section className="field-card wbb-ranking-card" aria-labelledby="wbb-rankings-title">
    ...
    {!publication ? <p className={error ? "status-error" : "muted"} role={error ? "alert" : "status"}>{error || "Loading women&apos;s player rankings…"}</p> : <>
      ...
      {board ? <>
        ...
        <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player</th><th>Team</th><th>Pos.</th><th className="numeric">Games</th>{usingBox ? <th className="numeric">Box rows</th> : null}{board.sample_unit ? <th className="numeric">Rate sample</th> : null}<th className="numeric">{board.unit}</th><th>Shot map</th></tr></thead><tbody>{rows.map((row) => <tr key={`${metric}-${row.player_id}`}><td className="rank-number">{row.rank}</td><th scope="row"><Link href={womensPlayerTableHref(row.player_id)}>{row.name}</Link><small>{usingBox ? "Exact box player " : "Source player "}{row.player_id} · <Link href={womensPlayerTableHref(row.player_id)}>open production</Link></small></th><td>{row.team}</td><td>{row.position || "—"}</td><td className="numeric">{row.games.toLocaleString()}</td>{usingBox ? <td className="numeric">{(row as RankingRow & { box_rows?: number }).box_rows?.toLocaleString() || "—"}</td> : null}{board.sample_unit ? <td className="numeric">{womensRankingSampleLabel(row, board) || "—"}</td> : null}<td className="numeric"><strong>{formatValue(row.value, board.unit)}</strong></td><td><Link className="text-link" href={womensPlayerShotMapHref(row.player_id)}>Review court →</Link></td></tr>)}</tbody></table></div>
