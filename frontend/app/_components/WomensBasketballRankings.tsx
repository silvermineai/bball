...
export default function WomensBasketballRankings() {
  ...
  return <section className="field-card wbb-ranking-card" aria-labelledby="wbb-rankings-title">
    ...
    {!publication ? <p className="muted">Loading women&apos;s player rankings…</p> : <>
      ...
      {board ? <>
        <p className="note">{board.description || board.label} · {coverage[metric]?.qualified.toLocaleString() || 0} qualified players · observed values {coverage[metric]?.observed.toLocaleString() || 0}. Showing {rows.length ? `${page * 50 + 1}–${page * 50 + rows.length}` : "0"} of {matchingRows.length.toLocaleString()} matching rows. Board ranks remain global when searching.</p>
        <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player</th><th>Team</th><th>Pos.</th><th className="numeric">Games</th>{usingBox ? <th className="numeric">Box rows</th> : null}<th className="numeric">{board.unit}</th></tr></thead><tbody>{rows.map((row) => <tr key={`${metric}-${row.player_id}`}><td className="rank-number">{row.rank}</td><th scope="row">{row.name}<small>{usingBox ? "Exact box player " : "Source player "}{row.player_id}</small></th><td>{row.team}</td><td>{row.position || "—"}</td><td className="numeric">{row.games.toLocaleString()}</td>{usingBox ? <td className="numeric">{(row as RankingRow & { box_rows?: number }).box_rows?.toLocaleString() || "—"}</td> : null}<td className="numeric"><strong>{formatValue(row.value, board.unit)}</strong></td></tr>)}</tbody></table></div>
