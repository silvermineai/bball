          ...
          {rows.map((game) => {
            ...
            return (
              <tr key={game.id}>
                ...
                <td className="numeric">{fmt(prediction.total)}{prediction.total_low == null || prediction.total_high == null ? null : <small>{fmt(prediction.total_low)} to {fmt(prediction.total_high)} · calibrated total range</small>}</td>
              </tr>
            );
          })}
        </tbody>
