...
export default function ScopeUnavailable({ sport, scope }: Props) {
  ...
  return (
    <section className="scope-unavailable" aria-labelledby="scope-unavailable-title">
      ...
      <p>{detail}</p>
      {womenRankings ? <WomensBasketballRankings /> : womenTeams ? <WomensBasketballTeams /> : womenPlayers ? <WomensBasketballPlayers /> : divisionPlayers ? <DivisionPlayerArchive division={scope.division} /> : isWomen && sport === "basketball" && scope.division === "1" ? <WomensBasketballSnapshot /> : null}
