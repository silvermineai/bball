...
import { rankWomensObservedPlayers, rankWomensRecruitingProspects, summarizeWomensRecruitingProspects, validateWomensRecruitingRelease, type WomensObservedMetric, type WomensObservedPlayer, type WomensRecruitingRelease } from "../_lib/womens-recruiting-intel";
...
export default function WomensBasketballRecruiting() {
  ...
  const prospects = useMemo(() => rankWomensRecruitingProspects(prospectEdition?.records || [], prospectQuery, prospectLimit), [prospectEdition?.records, prospectLimit, prospectQuery]);
  ...
  return <section className="field-card" aria-labelledby="wbb-recruiting-title">
    ...
    {!edition ? <p className="muted">Loading women&apos;s recruiting coverage…</p> : <>
      ...
      <section className="field-card wbb-player-card" aria-labelledby="wbb-recruiting-prospects-title">
        ...
        {prospectEdition ? <><div className="strip" aria-label="Women&apos;s prospect coverage"><div><strong>{prospectEdition.coverage.prospects.toLocaleString()}</strong><span>Prospects</span></div><div><strong>{prospectEdition.coverage.graded.toLocaleString()}</strong><span>Graded</span></div><div><strong>{prospectEdition.coverage.ranked.toLocaleString()}</strong><span>National ranks</span></div><div><strong>{prospectEdition.coverage.committed.toLocaleString()}</strong><span>Commitment IDs</span></div></div><p className="note">The source status mix is shown below for planning context. “Verbal” is retained publisher language; with zero destination IDs in this release, it is not treated as a school commitment or fit join.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Source status</th><th className="numeric">Prospects</th><th className="numeric">Grade coverage</th><th className="numeric">Average grade</th><th className="numeric">Exact IDs</th><th className="numeric">Destination IDs</th></tr></thead><tbody>{prospectStatusRows.map((row) => <tr key={row.status}><th scope="row">{row.status}</th><td className="numeric">{row.prospects.toLocaleString()}</td><td className="numeric">{row.graded.toLocaleString()}</td><td className="numeric">{row.averageGrade == null ? "—" : row.averageGrade.toFixed(1)}</td><td className="numeric">{row.exactIds.toLocaleString()}</td><td className="numeric">{row.destinationIds.toLocaleString()}</td></tr>)}</tbody></table></div><p className="note">Showing {prospects.length.toLocaleString()} of {prospectEdition.records.length.toLocaleString()} validated prospect rows. Choose “All retained” to inspect the complete source cohort.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Prospect</th><th>Position</th><th className="numeric">Grade</th><th>High school</th><th>Hometown</th><th>Status</th></tr></thead><tbody>{prospects.map((prospect) => <tr key={prospect.athlete_id}><th scope="row"><strong>{prospect.name}</strong><small>Exact source ID · {prospect.athlete_id}</small></th><td>{prospect.position || "—"}</td><td className="numeric">{prospect.grade == null ? "—" : prospect.grade}</td><td>{prospect.high_school || "—"}</td><td>{prospect.hometown || "—"}</td><td>{prospect.status || "—"}</td></tr>)}</tbody></table></div>{!prospects.length ? <p className="empty">No women&apos;s prospects match this search.</p> : null}<p className="note">Receipt-backed women&apos;s release edition <code>{prospectEdition.edition.slice(0, 12)}…</code> · captured {date(prospectEdition.captured_at)}. National and position ranks are unavailable for this release; the table is grade-sorted observed evidence, not a Silvermine ranking.</p></> : <p className="muted">Women&apos;s prospect release unavailable or failed its integrity checks; no rows are substituted.</p>}
      </section>
