...
export default function DivisionTeamArchive({ division }: { division: "2" | "3" }) {
  ...
  return <section className="field-card division-team-archive" aria-labelledby="division-team-title">
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !teams ? <p className="muted">Loading division team archive…</p> : <>
      ...
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Program</th><th>Conference</th><th className="numeric">GP</th><th className="numeric">W</th><th className="numeric">L</th><th className="numeric">Win%</th><th className="numeric">PPG</th></tr></thead><tbody>{rows.map((team) => <tr key={`${division}-${team.team_ncaa_id}`}><td className="rank-number">{team.rank == null ? "—" : team.rank}</td><th scope="row"><Link href={lowerDivisionTeamHref(division, team.team_ncaa_id)}>{team.name} →</Link><small>Team ID {team.team_ncaa_id}</small></th><td>{team.conference || "—"}</td><td className="numeric">{number(team.games)}</td><td className="numeric">{number(team.wins)}</td><td className="numeric">{number(team.losses)}</td><td className="numeric">{team.games && team.wins != null ? `${number(100 * team.wins / team.games, 1)}%` : "—"}</td><td className="numeric">{number(team.ppg, 1)}</td></tr>)}</tbody></table></div>
