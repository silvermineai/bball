...
export default function DivisionPlayerArchive({ division }: { division: "2" | "3" }) {
  ...
  return <section className="field-card division-player-archive" aria-labelledby="division-player-title">
    ...
    {!publication ? <p className="muted">Loading division player archive…</p> : <>
      ...
      <p className="note">Silvermine rank is calculated within the filtered Division {division} rows for the selected retained field ({metrics.find(({ key }) => key === metric)?.label || metric}); equal values share a rank. Missing values remain unranked.</p>
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Silvermine rank</th><th>Player</th><th>Team</th><th>Pos.</th><th className="numeric">GP</th><th className="numeric">PPG</th><th className="numeric">RPG</th><th className="numeric">APG</th><th className="numeric">SPG</th><th className="numeric">BPG</th><th className="numeric">MPG</th><th className="numeric">FG%</th><th className="numeric">3P%</th><th className="numeric">A/TO</th><th className="numeric">DD</th><th>Recorded line</th></tr></thead><tbody>{rows.map((player) => <tr key={`${division}-${player.player_id}`}><td className="rank-number">{player.archive_rank == null ? "—" : `#${player.archive_rank}`}</td><th scope="row"><Link href={lowerDivisionPlayerHref(division, player.player_id)}>{player.name} →</Link><small>Player ID {player.player_id}</small></th><td>{player.team_name || "—"}</td><td>{player.position || "—"}</td><td className="numeric">{value(player.games, 0)}</td><td className="numeric">{value(player.ppg)}</td><td className="numeric">{value(player.rpg)}</td><td className="numeric">{value(player.apg)}</td><td className="numeric">{value(player.spg)}</td><td className="numeric">{value(player.bpg)}</td><td className="numeric">{value(player.mpg)}</td><td className="numeric">{value(player.fg_pct)}</td><td className="numeric">{value(player.three_pct)}</td><td className="numeric">{value(player.ast_to)}</td><td className="numeric">{value(player.dbl_dbl, 0)}</td><td><details><summary>{retainedPlayerDetailCount(player)} recorded fields</summary><p className="note">Season values retained in the checked archive. A dash means this release did not contain a numeric value.</p>{divisionPlayerDetailGroups.map((group) => <div key={group.label}><strong>{group.label}</strong><div className="note">{group.fields.map(([key, label, kind]) => <span key={key} style={{ display: "inline-block", marginRight: 12 }}>{label}: <strong>{retainedPlayerValue(player, key) == null ? "—" : value(retainedPlayerValue(player, key), kind === "rate" ? 2 : kind === "minutes" ? 1 : 0)}</strong></span>)}</div></div>)}{player.source_stats && Object.keys(player.source_stats).length ? <p className="note">Retained source row evidence: {Object.entries(player.source_stats).map(([key, evidence]) => `${key}${evidence.rank == null ? "" : ` (#${evidence.rank})`}${evidence.value == null ? "" : ` = ${evidence.value}`}`).join(" · ")}</p> : null}</details></td></tr>)}</tbody></table></div>
