...
type Statistic = { label: string; statistic: string; headers: string[]; rows: Row[]; through_games?: string | null };
type Edition = { generated_at: string; source: { limitation: string }; divisions: Record<string, { source_scope: { division: number }; identity_status: string; identity_note: string; through_games?: string | null; individual: Statistic[]; team: Statistic[] }> };
...
export default function WomensLowerDivisionStats({ division }: { division: "2" | "3" }) {
  ...
  return <section className="field-card" aria-labelledby="wbb-lower-stats-title" style={{ marginTop: 18 }}>
    ...
    {!current ? <p className="muted">Loading the NCAA.com lower-division tables…</p> : <>
      ...
      <p className="muted">Capture receipts and the full available statistic selector are retained in the lower-division edition. No player ranking, forecast, or identity join is created from a name-only row.</p>
    </>}
  </section>;
}
