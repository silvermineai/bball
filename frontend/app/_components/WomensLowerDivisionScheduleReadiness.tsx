...
export default function WomensLowerDivisionScheduleReadiness({ division }: { division: "2" | "3" }) {
  ...
  return <div id="wbb-lower-schedule" className="paper-panel" style={{ marginTop: 18 }} aria-label={`Women’s D${division} schedule readiness`}>
    ...
    {!loaded ? <p className="muted">Checking for a retained D{division} schedule asset…</p> : error ? <p className="status-error">{error}</p> : !asset ? <>
      ...
      {targetProbe ? <div className="notice" style={{ marginBottom: 14 }}><strong>{targetProbe.season} target-season availability probe · D{division}</strong><p className="muted" style={{ margin: "4px 0 0" }}>The schedule endpoint was checked for months {targetProbe.months.join(", ")}: {targetProbe.divisionContests[division].toLocaleString()} D{division} contests and {targetProbe.divisionCalendarDays[division].toLocaleString()} D{division} calendar days returned across {targetProbe.receipts.toLocaleString()} receipt-backed responses ({new Date(targetProbe.generatedAt).toLocaleString()}). The combined probe found {targetProbe.contests.toLocaleString()} contests; only rows carrying this exact division are eligible for this panel. Empty responses remain unavailable data; predictions stay gated until a current exact-division schedule is published.</p>{targetProbe.unscopedCalendarDays || targetProbe.unscopedContests ? <p className="muted" style={{ margin: "4px 0 0" }}>Source-scope diagnostic: {targetProbe.unscopedCalendarDays.toLocaleString()} calendar rows and {targetProbe.unscopedContests.toLocaleString()} contests lacked an explicit D2/D3 division and are excluded.</p> : null}</div> : null}
