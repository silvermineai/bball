    ...
    <section className="paper-panel" aria-labelledby="lower-division-results-title" style={{ marginTop: 28 }}>
      ...
      {error ? <p className="status-error" role="alert">{error}</p> : !archive ? <p className="empty" role="status">Loading recorded lower-division results…</p> : <>
        ...
        {playerReadiness ? <div className="paper-panel" style={{ marginTop: 20 }} aria-label="Football lower-division player source readiness">
          ...
          <p className="note">{playerReadiness.event_archive ? `${playerReadiness.event_archive.players.toLocaleString()} exact ESPN athlete IDs across ${playerReadiness.event_archive.games.toLocaleString()} game summaries. ${playerReadiness.event_archive.classification} The canonical NCAA national-ranking contract remains separately blocked.` : `${playerReadiness.classification_policy} The ledger records public endpoint contracts without treating endpoint discovery as captured player data.`}</p>
