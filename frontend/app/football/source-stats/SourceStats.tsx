...
export default function SourceStats() {
  ...
  return (
    <>
      ...
      <section className="section paper-panel"><div className="eyebrow">Source boundary</div><h2>Everything stays in its namespace.</h2><p>the retained archive releases are attributed under CC BY 4.0 and retain publisher / licensed feed source fields. retained player rows add contest-level evidence while retaining the source name and team identifiers; they do not supply a stable athlete ID and are never joined to the player archive by name. This browser makes raw evidence discoverable without adding a composite grade or presenting archived betting rows as verified pregame lines.</p><p><Link href="/research/coverage/">Review source receipts and live row counts →</Link> · <Link href="/football/events/">Open the defense and specialist notebook →</Link></p></section>
    </>
  );
}
