...
type Result = { season: number | "all"; timing?: "all" | "pregame" | "postgame"; market?: "all" | "spreads" | "totals" | "h2h"; page: number; page_size: number; total: number; rows: Row[]; source?: string; unavailable_reason?: string; unavailable_sources?: string[] };
...
export default function Markets() {
  ...
  const [market, setMarket] = useState<"all" | "spreads" | "totals" | "h2h">("all");
  ...
  useEffect(() => {
    ...
    if (requestedMarket === "all" || requestedMarket === "spreads" || requestedMarket === "totals" || requestedMarket === "h2h") setMarket(requestedMarket);
  ...
  }, [hydrated, market, page, query, season, sport, timing]);
  ...
  useEffect(() => {
    ...
    setData(null);
    setError("");
    const params = new URLSearchParams({ sport, season, timing, market, page: String(page) });
  ...
  }, [market, sport, season, timing, query, page]);
  ...
  // same game and market.
  const headers = ["season", "away", "home", "kickoff", "market", "provider", "bookmaker", "home_spread", "total", "home_price_decimal", "away_price_decimal", "over_price_decimal", "under_price_decimal", "home_implied_probability", "observed_at", "updated_at", "source", "is_pregame"];
  const rowValues = (r: Row) => [r.season, r.away_name, r.home_name, r.kickoff, r.market, r.provider, r.bookmaker, r.home_spread, r.total, r.home_price, r.away_price, r.over_price, r.under_price, homeImplied(r), r.observed_at, r.updated_at, r.source, r.is_pregame];
  ...
  const downloadAll = async () => {
    ...
    if (totalPages > 1001) {
      setExportMessage("This filtered archive exceeds the bounded export window. Add a team or program filter first.");
      return;
    }
    setExporting(true);
    ...
    try {
      ...
      for (let requestedPage = 0; requestedPage < totalPages; requestedPage += 1) {
        const params = new URLSearchParams({ sport, season, timing, market, page: String(requestedPage) });
        if (query.trim()) params.set("q", query.trim());
    ...
    <section className="section" aria-label="Historical market archive">
      <div className="paper-panel brief-archive-note">
        ...
        {meta?.archive_receipts?.length ? <details className="note" style={{ marginTop: 14 }}>
          <summary>Retained archive receipts</summary>
          <div className="table-scroll" style={{ marginTop: 12 }}>
            <table className="data-table">
              <thead><tr><th>Dataset</th><th>Season</th><th>Fetched (UTC)</th><th>SHA-256</th></tr></thead>
              <tbody>{meta.archive_receipts.map((receipt) => <tr key={`${receipt.dataset}-${receipt.season}`}>
                <th>{receipt.dataset.replaceAll("_", " ")}</th>
      ...
      <div className="toolbar">
        ...
        <label className="control"><span>MARKET</span><select aria-label="Market type" value={market} onChange={(e) => { setMarket(e.target.value as typeof market); setPage(0); }}><option value="all">All market types</option><option value="spreads">Spreads</option><option value="totals">Totals</option><option value="h2h">Moneyline</option></select></label>
