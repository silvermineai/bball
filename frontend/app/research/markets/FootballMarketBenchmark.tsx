...
type Benchmark = { season: number; generated_at: string; methodology: string; coverage: { evaluation_games: number; market_games: number; pregame_market_games: number; timing_qualified_games?: number; market_source: string }; metrics: { model: BenchmarkMetric; archived_line: BenchmarkMetric; timing_qualified?: { games: number; model: BenchmarkMetric; archived_line: BenchmarkMetric } }; rows: BenchmarkRow[] };
const metric = (value: number | null, suffix = "") => value == null ? "—" : `${(suffix === "%" ? value * 100 : value).toFixed(2)}${suffix}`;

function TimingQualifiedPanel({ data }: { data: Benchmark }) {
  const timing = data.metrics.timing_qualified;
  const games = timing?.games ?? data.coverage.timing_qualified_games ?? data.coverage.pregame_market_games;
  return <section className="paper-panel" style={{ marginTop: 24 }} aria-labelledby="football-timing-qualified-title">
    <div className="section-heading"><div><div className="eyebrow">Timing-qualified cohort</div><h3 id="football-timing-qualified-title">Use only verified pregame evidence</h3></div><span className="note">{games.toLocaleString()} games</span></div>
    {games > 0 && timing ? <>
      <p className="note">These metrics use only rows whose retained source marked the observation as pregame. They are separate from the larger archive-reference cohort above.</p>
      <div className="stat-grid"><div><strong>{metric(timing.model.margin_mae)}</strong><span>model margin MAE</span></div><div><strong>{metric(timing.archived_line.margin_mae)}</strong><span>line margin MAE</span></div><div><strong>{metric(timing.model.winner_accuracy, "%")}</strong><span>model winner accuracy</span></div><div><strong>{metric(timing.archived_line.winner_accuracy, "%")}</strong><span>line winner accuracy</span></div></div>
    </> : <p className="note">No retrospective rows have a verified pregame capture clock. The archive-reference metrics remain descriptive; prospective comparisons require a timestamped market observation before kickoff.</p>}
  </section>;
}
