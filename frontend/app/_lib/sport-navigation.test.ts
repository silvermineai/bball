...
describe("sport navigation", () => {
  ...
  it("describes football lower-division coverage without overstating the archive", () => {
    ...
    expect(sportAvailabilityMessage("football", "2")).toBe(
      "Football coverage: D2 schedule rows, score-derived team records, exact-division ratings and validated forecasts are published. Lower-division player tables and player rankings remain unavailable; no D1 rows are substituted.",
    );
  ...
  it("makes women's basketball division boundaries visible in the shared nav", () => {
    ...
    expect(sportAvailabilityMessage("womens-basketball", "2")).toBe(
      "Women's basketball coverage: D2 source-native NCAA leaderboards are published for names and team slugs. Stable-ID player archives, rankings, and forecasts remain unavailable; no D1 rows are substituted.",
    );
  });
});
