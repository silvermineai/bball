...
import { teamStatCohortPercentile, teamStatCohortRank } from "../../_lib/team-stat-view";
...
type Result = { season: number; field: Field; page: number; page_size: number; total: number; non_null: number; source: SourceReceipt | null; rows: Row[] };
...
export default function TeamStats() {
  ...
  const exportHeaders = ["Cohort rank", "Cohort percentile", "Season", "Category", "Field", "Field unit", "Program", "Program ID", "Program archive URL", "Abbreviation", "Displayed value", "Raw numeric value"];
  const exportRow = (row: Row, active: Result, index: number) => {
    const rank = teamStatCohortRank(active.page, active.page_size, active.total, index);
    return [rank, teamStatCohortPercentile(rank, active.total), active.season, active.field.category, active.field.key, active.field.unit, row.team, row.id, `https://bball.silvermine.dev/basketball/programs/${encodeURIComponent(row.id)}/`, row.abbreviation, row.display, row.value];
  };
  const download = () => {
    if (!result) return;
    downloadCsv(`team-${result.field.key}-${season}-page-${page + 1}.csv`, toCsv(exportHeaders, result.rows.map((row, index) => exportRow(row, result, index))));
  };
  const downloadAll = async () => {
    ...
    try {
      ...
      downloadCsv(`team-${field.key}-${season}-all.csv`, toCsv(exportHeaders, rows.map((row, index) => exportRow(row, { ...result, page: 0, page_size: rows.length, total: rows.length }, index))));
