...
export default function TeamStats() {
  ...
  const exportRow = (row: Row, active: Result, index: number) => {
    const rank = teamStatCohortRank(active.page, active.page_size, active.total, index, row.rank);
  ...
  return <>
    ...
    <p className="note" style={{ marginBottom: 20 }}>
      {division === "1" ? "Division I rows are scoped by exact team IDs from the retained D1 index." : division === "all" ? "Publisher rows are shown without a division label; no division is inferred from team names." : `Division ${division} requires a source-native team-season release and remains unavailable when the publisher does not label rows.`}
    </p>
