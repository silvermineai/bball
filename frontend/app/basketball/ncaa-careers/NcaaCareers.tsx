...
const metricNote: Partial<Record<Metric, string>> = {
  ...
  ft_pct: "Free-throw accuracy is FTM ÷ FTA; the board withholds zero-attempt seasons.",
  per40: "Points per 40 scales recorded scoring by minutes. Keep the workload columns beside the rate.",
  stocks40: "Stocks per 40 combines recorded steals and blocks, scaled by minutes; it describes events rather than total defense.",
  ast_to: "Assist-to-turnover ratio uses recorded assists divided by turnovers; zero-turnover seasons remain unavailable.",
  tov_rate: "Turnover rate is recorded turnovers divided by recorded offensive possessions.",
  three_rate: "Three-point attempt rate is 3PA ÷ FGA and describes shot profile, not accuracy.",
  orb40: "Offensive rebounds per 40 scales recorded offensive boards by minutes.",
...
export default function NcaaCareers() {
  ...
  const exportHeaders = ["From season", "Through season", "Metric", "Minimum games", "Minimum minutes", "Minimum metric denominator", "Metric denominator field", "Rank", "Season", "Player", "NCAA player ID", "NCAA player source URL", "Program", "NCAA team ID", "NCAA team source URL", "Source class", "Source position", "Games", "Minutes", "Points", "Rebounds", "Offensive rebounds", "Defensive rebounds", "Assists", "Steals", "Blocks", "Turnovers", "Fouls", "Offensive possessions", "FGM", "FGA", "3PM", "3PA", "FTM", "FTA", "Value", "Season source release URLs", "Season source retrieved", "Season source SHA-256"];
  const exportRow = (row: Row, active: Result) => {
    const receipts = (active.source_receipts || []).filter((receipt) => receipt.season === row.season);
    return [active.from_season, active.to_season, labels[active.metric], active.min_games, active.min_minutes, active.min_denominator, active.denominator_field, row.rank, row.season, row.player_name, row.player_id, `https://stats.ncaa.org/players/${encodeURIComponent(row.player_id)}`, row.team_name, row.team_id, `https://stats.ncaa.org/teams/${encodeURIComponent(row.team_id)}`, row.class_year, row.position, row.games, row.minutes, row.points, row.rebounds, row.offensive_rebounds, row.defensive_rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.possessions, row.fgm, row.fga, row.tpm, row.tpa, row.ftm, row.fta, row.value, receipts.map((receipt) => receipt.url).join(" | ") || null, receipts.map((receipt) => receipt.fetched_at).join(" | ") || null, receipts.map((receipt) => receipt.sha256).join(" | ") || null];
  };
  const download = () => {
    if (!result) return;
    downloadCsv(
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>SOURCE CLASS</span><select value={classYear} onChange={(e) => reset(() => setClassYear(e.target.value))}><option value="">All classes</option>{classYear && !classOptions.includes(classYear) && <option value={classYear}>{classYear} · not in catalog</option>}{classOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
      <label className="control"><span>SOURCE POSITION</span><select value={position} onChange={(e) => reset(() => setPosition(e.target.value))}><option value="">All positions</option>{position && !positionOptions.includes(position) && <option value={position}>{position} · not in catalog</option>}{positionOptions.map((value) => <option key={value} value={value}>{value}</option>)}</select></label>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveArchive}>Retry live history</button></div> : !result ? <p className="empty" role="status">Loading career records…</p> : <>
      ...
      {!!result.source_receipts?.length && <details className="career-coverage-details" style={{ marginBottom: 20 }}><summary>Edition receipts for this season window ({result.source_receipts.length})</summary><p className="note">Each receipt identifies the retained edition behind the player-season rows. Retrieval clocks and hashes are shown exactly as recorded.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Dataset</th><th>Season</th><th>Retrieved (UTC)</th><th>SHA-256</th><th>Status</th></tr></thead><tbody>{result.source_receipts.map((receipt) => <tr key={`${receipt.dataset}-${receipt.season}`}><th scope="row">{receipt.dataset.replaceAll("_", " ")}</th><td>{seasonLabel(receipt.season)}</td><td>{new Date(receipt.fetched_at).toLocaleString("en-US", { timeZone: "UTC", dateStyle: "medium", timeStyle: "short" })}</td><td><code>{receipt.sha256}</code></td><td>Retained</td></tr>)}</tbody></table></div></details>}
      ...
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player</th><th>Season / program</th><th>Class / position</th><th className="numeric">GP</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">{labels[result.metric]}</th></tr></thead><tbody>{result.rows.map((row) => <tr key={`${row.season}-${row.player_id}-${row.team_id}`}><td className="numeric"><strong>#{row.rank}</strong></td><td><strong>{row.player_name || row.player_id}</strong><small>Archive player {row.player_id}</small><small><Link href={`/basketball/ncaa-player/?id=${encodeURIComponent(row.player_id)}&season=${row.season}`}>Open player card →</Link> · <Link href={`/basketball/ncaa-compare/?ids=${encodeURIComponent(row.player_id)}&season=${row.season}`}>Compare →</Link></small></td><td><strong>{row.team_name || row.team_id}</strong><small>{seasonLabel(row.season)} · team {row.team_id}</small></td><td><strong>{[row.class_year, row.position].filter(Boolean).join(" · ") || "—"}</strong><small>Roster metadata</small></td><td className="numeric">{fmt(row.games, 0)}</td><td className="numeric">{fmt(row.minutes, 0)}</td><td className="numeric">{fmt(row.points, 0)}</td><td className="numeric">{fmt(row.rebounds, 0)}</td><td className="numeric">{fmt(row.assists, 0)}</td><td className="numeric"><strong>{fmt(row.value, metricDigits(result.metric))}{percentMetrics.has(result.metric) ? "%" : ""}</strong></td></tr>)}</tbody></table></div>
      ...
      <p className="note" style={{ marginTop: 24 }}>The retained player box edition does not provide a stable cross-season player crosswalk, so this board keeps each player-season row separate rather than treating a name match as a verified career.</p>
    </>}
  </>;
}
