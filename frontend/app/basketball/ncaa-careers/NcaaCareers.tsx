...
export default function NcaaCareers() {
  ...
  const exportRow = (row: Row, active: Result) => {
    ...
    return [active.from_season, active.to_season, labels[active.metric], active.min_games, active.min_minutes, active.min_denominator, active.denominator_field, row.rank, row.season, row.player_name, row.player_id, `https://stats.ncaa.org/players/${encodeURIComponent(row.player_id)}`, row.team_name, row.team_id, `https://stats.ncaa.org/teams/${encodeURIComponent(row.team_id)}`, row.games, row.minutes, row.points, row.rebounds, row.offensive_rebounds, row.defensive_rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.possessions, row.fgm, row.fga, row.tpm, row.tpa, row.ftm, row.fta, row.value, receipts.map((receipt) => receipt.url).join(" | ") || null, receipts.map((receipt) => receipt.fetched_at).join(" | ") || null, receipts.map((receipt) => receipt.sha256).join(" | ") || null];
  };
  const download = () => {
    if (!result) return;
    downloadCsv(
