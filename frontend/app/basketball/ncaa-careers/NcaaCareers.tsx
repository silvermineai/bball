...
export default function NcaaCareers() {
  ...
  const exportHeaders = ["From season", "Through season", "Metric", "Minimum games", "Minimum minutes", "Minimum metric denominator", "Metric denominator field", "Rank", "Season", "Player", "Archive player ID", "Program", "Archive team ID", "Class", "Position", "Games", "Minutes", "Points", "Rebounds", "Offensive rebounds", "Defensive rebounds", "Assists", "Steals", "Blocks", "Turnovers", "Fouls", "Offensive possessions", "FGM", "FGA", "3PM", "3PA", "FTM", "FTA", "Value", "Edition retrieved", "Edition SHA-256"];
  const exportRow = (row: Row, active: Result) => {
    const receipts = (active.source_receipts || []).filter((receipt) => receipt.season === row.season);
    return [active.from_season, active.to_season, labels[active.metric], active.min_games, active.min_minutes, active.min_denominator, active.denominator_field, row.rank, row.season, row.player_name, row.player_id, row.team_name, row.team_id, row.class_year, row.position, row.games, row.minutes, row.points, row.rebounds, row.offensive_rebounds, row.defensive_rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.possessions, row.fgm, row.fga, row.tpm, row.tpa, row.ftm, row.fta, row.value, receipts.map((receipt) => receipt.fetched_at).join(" | ") || null, receipts.map((receipt) => receipt.sha256).join(" | ") || null];
  };
  const download = () => {
    if (!result) return;
    downloadCsv(
