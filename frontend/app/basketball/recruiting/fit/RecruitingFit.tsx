...
export default function RecruitingFit({ teams }: { teams: FitTeam[] }) {
  ...
  const csv = () => downloadCsv(`basketball-recruiting-fit-${teamId}.csv`, toCsv(["Rank", "Player", "Source ID", "Current source-listed program", "Role", "Status", "Prior programs", "Prior minutes", "Prior MPG", "Prior PPG", "Prior RPG", "Prior APG", "Prior TS%", "Prior eFG%", "Skill percentile", "Workload percentile", "Fit score", "Source URL"], result.map((row, index) => [index + 1, row.player.name, row.player.id, row.player.team, displayRole(row.role), row.player.status, row.player.previous_teams.join("; "), row.player.prior_production?.minutes, row.player.prior_production?.mpg, row.player.prior_production?.ppg, row.player.prior_production?.rpg, row.player.prior_production?.apg, row.player.prior_production?.ts == null ? null : row.player.prior_production.ts * 100, row.player.prior_production?.efg == null ? null : row.player.prior_production.efg * 100, row.skillPercentile, row.workloadPercentile, row.score, row.player.source_url])));
      ...
      {error ? <p className="status-error" role="alert">{error}</p> : !data ? <p className="empty" role="status">Loading source-listed roster observations…</p> : (
        <>
          <section className="section" aria-label="Selected program role room">
            ...
            <div className="fit-role-grid">{summaries.map((summary) => <article className={`paper-panel ${role === summary.role ? "fit-role-active" : ""}`} key={summary.role}><div className="eyebrow">{roleLabels[summary.role]}</div><strong className="fit-role-number">{summary.listed}</strong><span>listed players</span><dl><div><dt>Prior minutes</dt><dd>{Math.round(summary.priorMinutes).toLocaleString()}</dd></div><div><dt>Returning</dt><dd>{Math.round(summary.returningMinutes).toLocaleString()}</dd></div><div><dt>Incoming</dt><dd>{Math.round(summary.incomingMinutes).toLocaleString()}</dd></div></dl><p className="note">{summary.topPlayers.length ? summary.topPlayers.map((player) => `${player.name} · ${Math.round(player.prior_production?.minutes || 0).toLocaleString()} min`).join(" · ") : "No matched prior minutes in this role."}</p></article>)}</div>
          </section>
          ...
          {selected.length > 0 && <section className="section" aria-label="Shortlist comparison"><div className="section-heading"><div><div className="eyebrow">Shortlist comparison / prior source production</div><h2>Put the candidates side by side.</h2></div><span className="note">Observed prior season · exact source IDs</span></div><p className="note">These columns compare the same retained prior-season fields used by the fit board. Missing values remain unavailable; none predicts a new-school role or establishes eligibility.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Candidate</th><th>Role</th><th className="numeric">Fit</th><th className="numeric">MPG</th><th className="numeric">PPG</th><th className="numeric">RPG</th><th className="numeric">APG</th><th className="numeric">TS%</th><th className="numeric">eFG%</th><th className="numeric">Box BPM</th></tr></thead><tbody>{selected.map((row) => { const production = row!.player.prior_production; return <tr key={`compare-${row!.player.id}`}><th scope="row"><Link href={`/basketball/player/?id=${encodeURIComponent(row!.player.id)}&season=${data.previous_season}`}>{row!.player.name}</Link><small>{row!.player.team}</small></th><td>{displayRole(row!.role)}</td><td className="numeric"><strong>{fmt(row!.score, 1)}</strong></td><td className="numeric">{fmt(production?.mpg)}</td><td className="numeric">{fmt(production?.ppg)}</td><td className="numeric">{fmt(production?.rpg)}</td><td className="numeric">{fmt(production?.apg)}</td><td className="numeric">{production?.ts == null ? "—" : `${fmt(production.ts * 100)}%`}</td><td className="numeric">{production?.efg == null ? "—" : `${fmt(production.efg * 100)}%`}</td><td className="numeric">{fmt(production?.box_bpm)}</td></tr>; })}</tbody></table></div></section>}
