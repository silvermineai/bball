  ...
  return (
    <>
      <section className="board-priorities fit-priorities" aria-label="Recruiting fit controls">
        ...
        {sourceReceipt && (sourceReceipt.url || sourceReceipt.sha256 || sourceReceipt.fetched_at) && <details className="note" style={{ marginTop: 12 }}><summary>Fit board edition receipt</summary><div className="table-scroll" style={{ marginTop: 10 }}><table className="data-table"><thead><tr><th>Dataset</th><th>Retrieved (UTC)</th><th>SHA-256</th><th>Status</th></tr></thead><tbody><tr><td>{sourceReceipt.dataset.replaceAll("_", " ")}</td><td>{sourceReceipt.fetched_at ? new Date(sourceReceipt.fetched_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }) : "—"}</td><td><code>{sourceReceipt.sha256 || "—"}</code></td><td>Retained</td></tr></tbody></table></div><p style={{ marginTop: 10 }}>The receipt identifies the edition behind this shortlist. It does not establish eligibility, transfer status or a projected role.</p></details>}
