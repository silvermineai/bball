...
export default function RecruitingFit({ teams }: { teams: FitTeam[] }) {
  ...
  const [teamId, setTeamId] = useState(fallbackTeam), [role, setRole] = useState<FitRole>("any"), [focus, setFocus] = useState<FitFocus>("creation"), [minimumMinutes, setMinimumMinutes] = useState(400), [query, setQuery] = useState(""), [page, setPage] = useState(0), [picked, setPicked] = useState<string[]>([]), [hydrated, setHydrated] = useState(false), [copied, setCopied] = useState("");
    ...
    <>
      <section className="board-priorities fit-priorities" aria-label="Recruiting fit controls">
        ...
        <div className="toolbar board-filters fit-filters">
          <label className="control"><span>PROGRAM</span><select value={teamId} onChange={(event) => reset(() => setTeamId(event.target.value))}>{teams.slice().sort((a, b) => (a.rank || 999) - (b.rank || 999) || a.name.localeCompare(b.name)).map((team) => <option key={team.id} value={team.id}>{team.rank ? `#${team.rank} ` : ""}{team.name}</option>)}</select></label>
        ...
        <div className="button-row" style={{ marginTop: 14 }}><button className="button secondary" type="button" onClick={share}>Copy fit board link</button>{copied && <span className="note" role="status">{copied}</span>}</div>
      </section>
      {error ? <p className="status-error" role="alert">{error}</p> : !data ? <p className="empty" role="status">Loading source-listed roster observations…</p> : (
        <>
          ...
          <section className="section" aria-label="Recruiting candidate results">
            ...
            <div className="board-shortlist-summary"><span><strong>{picked.length} of 5 shortlisted</strong>{selected.length > 0 && <small>{selected.map((row) => row!.player.name).join(" · ")}</small>}</span><span className="note">Ranked by transparent fit score</span></div>
