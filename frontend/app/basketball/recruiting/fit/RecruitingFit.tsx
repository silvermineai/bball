...
export default function RecruitingFit({ teams }: { teams: FitTeam[] }) {
  ...
  const csv = () => downloadCsv(`basketball-recruiting-fit-${teamId}.csv`, toCsv(["Rank", "Player", "Source ID", "Current source-listed program", "Current team ID", "Role", "Status", "Class", "Height", "Weight", "Prior programs", "Prior minutes", "Prior starts", "Starter reported", "Reported starter rate", "Prior MPG", "Prior PPG", "Prior RPG", "Prior APG", "Prior TS%", "Prior eFG%", "Skill percentile", "Workload percentile", "Fit score", "Source URL", "Private note", "Roster source dataset", "Roster release URL", "Roster retrieved (UTC)", "Roster SHA-256"], result.map((row, index) => [index + 1, row.player.name, row.player.id, row.player.team, row.player.team_id, displayRole(row.role), row.player.status, row.player.class_year, row.player.height, row.player.weight, row.player.previous_teams.join("; "), row.player.prior_production?.minutes, row.player.prior_production?.starts, row.player.prior_production?.starter_reported_records, row.player.prior_production?.starter_rate == null ? null : row.player.prior_production.starter_rate * 100, row.player.prior_production?.mpg, row.player.prior_production?.ppg, row.player.prior_production?.rpg, row.player.prior_production?.apg, row.player.prior_production?.ts == null ? null : row.player.prior_production.ts * 100, row.player.prior_production?.efg == null ? null : row.player.prior_production.efg * 100, row.skillPercentile, row.workloadPercentile, row.score, row.player.source_url, notes[row.player.id] || "", sourceReceipt?.dataset, sourceReceipt?.url, sourceReceipt?.fetched_at, sourceReceipt?.sha256])));
