      ...
      {error ? <p className="status-error" role="alert">{error}</p> : !prospect ? <p className="empty" role="status">Loading exact prospect record…</p> : (
        <>
          ...
          {history.length > 1 && <section className="paper-panel" aria-label="Prospect rank history" style={{ marginBottom: 24 }}>
            ...
            <div className="table-scroll"><table className="data-table"><thead><tr><th>Captured</th><th className="numeric">National rank</th><th className="numeric">Position rank</th><th className="numeric">State rank</th><th className="numeric">Region rank</th><th className="numeric">Change</th><th className="numeric">Grade</th><th>Commitment / status</th><th>Capture</th></tr></thead><tbody>{history.map((entry, index) => {
              const prior = history[index - 1];
              const change = prior?.rank != null && entry.rank != null ? prior.rank - entry.rank : null;
              return <tr key={`${entry.edition}-${entry.captured_at}`}><td>{entry.captured_at ? new Date(entry.captured_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }) : "—"}<small>{entry.edition}</small></td><td className="numeric"><strong>{rank(entry.rank)}</strong></td><td className="numeric">{rank(entry.position_rank ?? null)}</td><td className="numeric">{rank(entry.state_rank ?? null)}</td><td className="numeric">{rank(entry.region_rank ?? null)}</td><td className={`numeric${change == null ? "" : change > 0 ? " movement-up" : change < 0 ? " movement-down" : ""}`}>{change == null ? "—" : change > 0 ? `▲ ${change}` : change < 0 ? `▼ ${Math.abs(change)}` : "= 0"}</td><td className="numeric">{entry.grade == null || entry.grade <= 0 ? "—" : number(entry.grade, 1)}</td><td>{entry.committed_team_name || entry.status || "—"}</td><td><small>{entry.captured_at ? new Date(entry.captured_at).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }) : "Capture date unavailable"}</small><small className="source-hash">{entry.edition}</small></td></tr>;
            })}</tbody></table></div>
          </section>}
          <div className="strip">
