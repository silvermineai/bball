  ...
  return <>
    ...
    <section className="section">
      ...
      <div className="toolbar">
        ...
        <a className="button secondary" href={`/api/basketball/research/matchup-stints/source?season=${encodeURIComponent(String(season))}`}>Download matchup archive ↓</a>
      </div>
      ...
      {error ? <p role="alert" className="status-error">{error}</p> : !edition ? <p role="status" className="empty">Loading matchup rows…</p> : <>
        ...
        {!visible.length && <p className="empty">No recorded pairs match these filters.</p>}
      </>}
    </section>
    <section className="section paper-panel"><h2>How to use the archive.</h2><p>These are recorded stints, not a projection. The public edition keeps the 5,000 highest-possession lineup pairs per season; possessions and games remain visible so a small sample does not look like a stable matchup. “Home edge / 100” is home points minus away points per 100 shared possessions.</p><p>Player and team labels remain archive-native and are not joined to a universal identity graph. Retained editions and retrieval receipts stay available in the research archive for reproducibility.</p></section>
  </>;
}
