...
const sortLabels: Record<RosterBoardSort, string> = {
  ...
  box_bpm: "Prior publisher Box BPM",
  apg: "Prior assists per game",
  rpg: "Prior rebounds per game",
  orpg: "Prior offensive rebounds per game",
  drpg: "Prior defensive rebounds per game",
  fpg: "Prior fouls per game",
};
...
export default function RosterBoard() {
    ...
    downloadCsv(
      ...
      toCsv(
        ...
        rows.map((row) => [
          ...
          row.prior_production?.orpg,
          row.prior_production?.drpg,
          row.prior_production?.apg,
          row.prior_production?.fpg,
  ...
  return (
    <>
      ...
      <p className="note">Sorted by {sortLabels[sort]}. “Same program” and “different program” describe source-ID prior records; “new to dataset” means no prior appearance was found in this archive. Missing production remains missing. Box BPM is source-attributed and is not a Silvermine forecast input.</p>
      ...
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player / program</th><th>Roster observation</th><th>Prior workload</th><th className="numeric">PPG</th><th className="numeric">RPG</th><th className="numeric">ORB/G</th><th className="numeric">DRB/G</th><th className="numeric">APG</th><th className="numeric">PF/G</th><th className="numeric">eFG%</th><th className="numeric">TS%</th><th className="numeric">Box BPM</th><th>Evidence</th></tr></thead><tbody>{pageRows.map((row) => <tr key={`${row.id}-${row.team_id}`}><td className="rank-number">{row.rank ?? "—"}</td><th scope="row"><Link href={`/basketball/player/?id=${encodeURIComponent(row.id)}&season=2026`}>{row.name}</Link><small><Link href={`/basketball/programs/${encodeURIComponent(row.team_id)}/`}>{row.team}</Link> · {row.position || "Position unavailable"}</small><small><Link href={`/basketball/recruiting/?q=${encodeURIComponent(row.name)}`}>Search dated evidence →</Link></small></th><td>{statusLabels[row.status as RosterBoardStatus] || row.status}<small>{row.previous_teams.join(", ") || "No prior program listed"}</small></td><td>{row.prior_production ? <><strong>{fmt(row.prior_production.mpg)} MPG</strong><small>{Math.round(row.prior_production.minutes).toLocaleString()} min · {row.prior_production.games} GP · {row.workload_label}</small></> : "No prior record"}</td><td className="numeric">{fmt(row.prior_production?.ppg)}</td><td className="numeric">{fmt(row.prior_production?.rpg)}</td><td className="numeric">{fmt(row.prior_production?.orpg)}</td><td className="numeric">{fmt(row.prior_production?.drpg)}</td><td className="numeric">{fmt(row.prior_production?.apg)}</td><td className="numeric">{fmt(row.prior_production?.fpg)}</td><td className="numeric">{row.prior_production?.efg == null ? "—" : `${fmt(row.prior_production.efg * 100)}%`}</td><td className="numeric">{row.prior_production?.ts == null ? "—" : `${fmt(row.prior_production.ts * 100)}%`}</td><td className="numeric">{fmt(row.prior_production?.box_bpm)}</td><td>{row.source_url ? <a href={row.source_url} target="_blank" rel="noreferrer">Roster source ↗</a> : "No source link"}</td></tr>)}</tbody></table></div>
