...
function SourceTotals({ stats }: { stats: Stats }) {
  const fields = Object.entries(stats).sort(([a], [b]) => a.localeCompare(b));
  return <details className="paper-panel" style={{ marginTop: 24 }}>
    <summary>Open all {fields.length} retained source totals</summary>
    <p className="note" style={{ marginTop: 12 }}>
      These are the numeric fields retained by the NCAA release for this
      player-season row. Labels are presentation helpers; the source key and
      raw value remain visible, and unavailable fields stay unavailable.
    </p>
...
export default function NcaaPlayerCard() {
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !card ? <p className="empty" role="status">Loading source-native player evidence…</p> : <>
      ...
      {selectedRow && <SourceTotals stats={selectedRow.stats} />}
