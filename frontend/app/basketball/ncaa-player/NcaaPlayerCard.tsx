...
export default function NcaaPlayerCard() {
  ...
  return <>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryPlayerCard}>Retry player card</button></div> : !card ? <p className="empty" role="status">Loading player evidence…</p> : <>
      ...
      <section className="section two-col"><div className="paper-panel"><div className="eyebrow">Shot profile / {label(season)}</div><h2>Where attempts came from.</h2>{shootingRows.length ? <><p className="note">The archive publishes {shootingRows.length} team shooting row{shootingRows.length === 1 ? "" : "s"} for this season; each stint stays visible.</p>{shotLocations.length ? <PlayerShotLocationCourt shots={shotLocations} playerName={name} title="Recorded NCAA shot locations" showEvents /> : <p className="empty">This profile has aggregate shooting totals, but no retained coordinate rows for this season.</p>}{shootingRows.map((row) => <ShotProfile key={`${row.season}-${row.team_id}`} row={row} />)}</> : <p className="empty">No shooting profile is published for this season-team record.</p>}</div><div className="paper-panel"><div className="eyebrow">Roster / recruiting context</div><h2>What the roster release says.</h2>{roster ? <><div className="raw-stat-grid">{[["Class", roster.profile.class], ["Position", roster.profile.position], ["Height", roster.profile.height], ["Hometown", roster.profile.hometown], ["High school", roster.profile.high_school]].map(([key, field]) => <div key={key}><dt>{key}</dt><dd>{field || "—"}</dd></div>)}</div><div className="hero-actions"><Link className="button secondary" href={`/basketball/recruiting/?q=${encodeURIComponent(name)}`}>Search dated school evidence ↗</Link>{roster.profile.high_school && <Link className="hero-link" href={`/basketball/ncaa-high-schools/?q=${encodeURIComponent(String(roster.profile.high_school))}`}>Trace high-school pipeline →</Link>}</div><p className="note">These links are text searches for review. A name or school-label match does not establish identity, commitment, transfer, eligibility or current membership.</p></> : <p className="empty">No roster row is published for {label(season)}.</p>}</div></section>
      ...
      <section className="section"><div className="section-heading"><div><div className="eyebrow">Game evidence / {label(season)}</div><h2>Open the possessions behind the total.</h2></div><div className="button-row"><span className="note">{allGames ? `${visibleGames.length} rows loaded` : "Latest 12 retained rows"}</span>{allGames ? <button className="button secondary" type="button" onClick={() => setAllGames(null)}>Show latest 12</button> : <button className="button secondary" type="button" onClick={() => { void fetchFullGameLog(); }} disabled={loadingGames || !card.games.length}>{loadingGames ? "Loading log…" : "Show complete log"}</button>}<button className="button secondary" type="button" onClick={downloadFullGameLog} disabled={exporting || loadingGames || !card.games.length}>{exporting ? "Preparing log…" : "Download full season CSV ↓"}</button></div></div><p className="note">The card starts with the latest 12 rows for a fast load. Show complete log loads every retained contest for this player ID and season; the download includes all retained fields, including fields not displayed in the table.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Date / opponent</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">TO</th><th className="numeric">PF</th><th className="numeric">FG</th><th className="numeric">3P</th><th className="numeric">FT</th></tr></thead><tbody>{visibleGames.map((game) => <tr key={game.contest_id}><td>{game.game_date || "Date unavailable"}<small>{game.team_name || team} vs {game.opponent_name || "Opponent unavailable"} · contest {game.contest_id}</small></td><td className="numeric">{fmt(value(game.stats, "mins"), 0)}</td><td className="numeric">{fmt(value(game.stats, "pts"), 0)}</td><td className="numeric">{fmt(safeSum(value(game.stats, "orb"), value(game.stats, "drb")), 0)}</td><td className="numeric">{fmt(value(game.stats, "ast"), 0)}</td><td className="numeric">{fmt(value(game.stats, "tov"), 0)}</td><td className="numeric">{fmt(value(game.stats, "pf"), 0)}</td><td className="numeric">{fmt(value(game.stats, "fgm"), 0)}/{fmt(value(game.stats, "fga"), 0)}</td><td className="numeric">{fmt(value(game.stats, "tpm"), 0)}/{fmt(value(game.stats, "tpa"), 0)}</td><td className="numeric">{fmt(value(game.stats, "ftm"), 0)}/{fmt(value(game.stats, "fta"), 0)} <small>{pct(rate(value(game.stats, "ftm"), value(game.stats, "fta")))}</small></td></tr>)}</tbody></table></div>{!visibleGames.length && <p className="empty">No game-level rows are published for this season; use the season summary above.</p>}</section>
