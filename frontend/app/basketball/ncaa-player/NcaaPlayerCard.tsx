...
type Card = { player_id: string; selected_season: number; seasons: SeasonRow[]; rosters: RosterRow[]; shooting: ShotRow[]; games: GameRow[]; identity_note: string };
...
export default function NcaaPlayerCard() {
  ...
  useEffect(() => {
    ...
    setCard(null); setImpact(null); setError("");
    Promise.all([
  ...
  const downloadFullGameLog = async () => {
    ...
    try {
      ...
      const statKeys = Array.from(new Set(payload.rows.flatMap((row) => Object.keys(row.stats)))).sort();
      downloadCsv(
        ...
        toCsv(
          ...
          payload.rows.map((row) => [
            payload.season, row.game_date, row.contest_id, row.player_name, id, row.team_name, row.team_id, row.opponent_name,
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !card ? <p className="empty" role="status">Loading source-native player evidence…</p> : <>
      ...
      <section className="section"><div className="section-heading"><div><div className="eyebrow">Game evidence / {label(season)}</div><h2>Open the possessions behind the total.</h2></div><div className="button-row"><span className="note">Latest 12 source rows</span><button className="button secondary" type="button" onClick={downloadFullGameLog} disabled={exporting || !card.games.length}>{exporting ? "Preparing log…" : "Download full season CSV ↓"}</button></div></div><p className="note">The table shows the latest 12 rows for a fast card load. The download retrieves every retained contest for this NCAA player ID and season, including source fields that are not displayed in the table.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Date / opponent</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">FG</th><th className="numeric">3P</th></tr></thead><tbody>{card.games.map((game) => <tr key={game.contest_id}><td>{game.game_date || "Date unavailable"}<small>{game.team_name || team} vs {game.opponent_name || "Opponent unavailable"} · contest {game.contest_id}</small><small><a href={`https://stats.ncaa.org/game/index/${encodeURIComponent(game.contest_id)}`} target="_blank" rel="noreferrer">NCAA source ↗</a></small></td><td className="numeric">{fmt(value(game.stats, "mins"), 0)}</td><td className="numeric">{fmt(value(game.stats, "pts"), 0)}</td><td className="numeric">{fmt((value(game.stats, "orb") || 0) + (value(game.stats, "drb") || 0), 0)}</td><td className="numeric">{fmt(value(game.stats, "ast"), 0)}</td><td className="numeric">{fmt(value(game.stats, "fgm"), 0)}/{fmt(value(game.stats, "fga"), 0)}</td><td className="numeric">{fmt(value(game.stats, "tpm"), 0)}/{fmt(value(game.stats, "tpa"), 0)}</td></tr>)}</tbody></table></div>{!card.games.length && <p className="empty">No game-level rows are published for this season; use the season summary above.</p>}</section>
