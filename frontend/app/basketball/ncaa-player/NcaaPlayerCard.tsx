...
export default function NcaaPlayerCard() {
  ...
  const points = stat(selectedRow, "pts"), games = selectedRow?.games || 0, minutes = stat(selectedRow, "mins"), fga = stat(selectedRow, "fga"), fgm = stat(selectedRow, "fgm"), fta = stat(selectedRow, "fta"), ast = stat(selectedRow, "ast"), turnovers = stat(selectedRow, "tov"), fouls = stat(selectedRow, "pf"), orb = stat(selectedRow, "orb"), drb = stat(selectedRow, "drb");
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !card ? <p className="empty" role="status">Loading source-native player evidence…</p> : <>
      ...
      <section className="section"><div className="section-heading"><div><div className="eyebrow">Selected season / Production</div><h2>What the source recorded.</h2></div><span className="note">{selected?.length || 0} team record{selected?.length === 1 ? "" : "s"}</span></div><div className="strip"><div><strong>{fmt(minutes == null || !games ? null : minutes / games)}</strong><span>Minutes per game</span></div><div><strong>{fmt(safeSum(orb, drb) == null || !games ? null : safeSum(orb, drb)! / games)}</strong><span>Rebounds per game</span></div><div><strong>{fmt(ast == null || !games ? null : ast / games)}</strong><span>Assists per game</span></div><div><strong>{fmt(turnovers == null || !games ? null : turnovers / games)}</strong><span>Turnovers per game</span></div><div><strong>{fmt(fouls == null || !games ? null : fouls / games)}</strong><span>Fouls per game</span></div><div><strong>{pct(efg)}</strong><span>Effective FG%</span></div></div><div className="table-scroll"><table className="data-table"><thead><tr><th>Season / team</th><th className="numeric">GP</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">TO</th><th className="numeric">PF</th><th className="numeric">FG</th><th className="numeric">3P</th><th className="numeric">FT</th></tr></thead><tbody>{card.seasons.map((row) => <tr key={`${row.season}-${row.team_id}`} className={row.season === season ? "career-selected-row" : ""}><td><button className="career-season-link" onClick={() => setSeason(row.season)}>{label(row.season)} · {row.team_name || row.team_id} →</button><small>NCAA team {row.team_id}</small></td><td className="numeric">{row.games}</td><td className="numeric">{fmt(value(row.stats, "pts"), 0)}</td><td className="numeric">{fmt(safeSum(value(row.stats, "orb"), value(row.stats, "drb")), 0)}</td><td className="numeric">{fmt(value(row.stats, "ast"), 0)}</td><td className="numeric">{fmt(value(row.stats, "tov"), 0)}</td><td className="numeric">{fmt(value(row.stats, "pf"), 0)}</td><td className="numeric">{fmt(value(row.stats, "fgm"), 0)}/{fmt(value(row.stats, "fga"), 0)}</td><td className="numeric">{fmt(value(row.stats, "tpm"), 0)}/{fmt(value(row.stats, "tpa"), 0)}</td><td className="numeric">{fmt(value(row.stats, "ftm"), 0)}/{fmt(value(row.stats, "fta"), 0)}</td></tr>)}</tbody></table></div></section>
