...
function SourceTotals({ rows }: { rows: SeasonRow[] }) {
  ...
  return <details className="paper-panel" style={{ marginTop: 24 }}>
    <summary>Open {fieldCount} retained source totals across {rows.length} team row{rows.length === 1 ? "" : "s"}</summary>
    <p className="note" style={{ marginTop: 12 }}>
      These are the numeric fields retained by the NCAA release for every
    ...
    {rows.map((row) => <div key={`${row.season}-${row.team_id}`} style={{ marginTop: 18 }}>
      <h3>{row.team_name || row.team_id}<small style={{ marginLeft: 8 }}>NCAA team {row.team_id}</small></h3>
...
export default function NcaaPlayerCard() {
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !card ? <p className="empty" role="status">Loading source-native player evidence…</p> : <>
      ...
      <section className="section"><div className="section-heading"><div><div className="eyebrow">Game evidence / {label(season)}</div><h2>Open the possessions behind the total.</h2></div><div className="button-row"><span className="note">{allGames ? `${visibleGames.length} source rows loaded` : "Latest 12 source rows"}</span>{allGames ? <button className="button secondary" type="button" onClick={() => setAllGames(null)}>Show latest 12</button> : <button className="button secondary" type="button" onClick={() => { void fetchFullGameLog(); }} disabled={loadingGames || !card.games.length}>{loadingGames ? "Loading log…" : "Show complete log"}</button>}<button className="button secondary" type="button" onClick={downloadFullGameLog} disabled={exporting || loadingGames || !card.games.length}>{exporting ? "Preparing log…" : "Download full season CSV ↓"}</button></div></div><p className="note">The card starts with the latest 12 rows for a fast load. Show complete log loads every retained contest for this NCAA player ID and season; the download includes all source fields, including fields not displayed in the table.</p><div className="table-scroll"><table className="data-table"><thead><tr><th>Date / opponent</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">TO</th><th className="numeric">PF</th><th className="numeric">FG</th><th className="numeric">3P</th><th className="numeric">FT</th></tr></thead><tbody>{visibleGames.map((game) => <tr key={game.contest_id}><td>{game.game_date || "Date unavailable"}<small>{game.team_name || team} vs {game.opponent_name || "Opponent unavailable"} · contest {game.contest_id}</small><small><a href={`https://stats.ncaa.org/game/index/${encodeURIComponent(game.contest_id)}`} target="_blank" rel="noreferrer">NCAA source ↗</a></small></td><td className="numeric">{fmt(value(game.stats, "mins"), 0)}</td><td className="numeric">{fmt(value(game.stats, "pts"), 0)}</td><td className="numeric">{fmt(safeSum(value(game.stats, "orb"), value(game.stats, "drb")), 0)}</td><td className="numeric">{fmt(value(game.stats, "ast"), 0)}</td><td className="numeric">{fmt(value(game.stats, "tov"), 0)}</td><td className="numeric">{fmt(value(game.stats, "pf"), 0)}</td><td className="numeric">{fmt(value(game.stats, "fgm"), 0)}/{fmt(value(game.stats, "fga"), 0)}</td><td className="numeric">{fmt(value(game.stats, "tpm"), 0)}/{fmt(value(game.stats, "tpa"), 0)}</td><td className="numeric">{fmt(value(game.stats, "ftm"), 0)}/{fmt(value(game.stats, "fta"), 0)} <small>{pct(rate(value(game.stats, "ftm"), value(game.stats, "fta")))}</small></td></tr>)}</tbody></table></div>{!visibleGames.length && <p className="empty">No game-level rows are published for this season; use the season summary above.</p>}</section>
