...
export default function NcaaPlayerCard() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`/api/basketball/research/ncaa-player-card/${encodeURIComponent(id)}?season=${season}`, { signal: controller.signal }).then(async (r) => { if (!r.ok) throw Error(r.status === 404 ? "No archive record was found for that player ID." : "The player card could not be loaded."); return r.json() as Promise<Card>; }),
