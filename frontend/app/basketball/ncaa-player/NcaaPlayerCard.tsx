...
export default function NcaaPlayerCard() {
  ...
  return <>
    ...
    <div className="page-title"><div className="eyebrow">Player card / archive ID {id}</div><h1>{name}</h1><p>{team} · {label(season)}. This card keeps the archive identity namespace intact while bringing production, shot selection, roster fields and impact evidence together. It does not assert current eligibility or join records by name.</p><div className="hero-actions"><Link className="hero-link" href={`/basketball/ncaa-player-box/?season=${season}&q=${encodeURIComponent(id)}`}>Open box archive rows →</Link><Link className="hero-link" href={`/basketball/players/?q=${encodeURIComponent(name)}`}>Search player archive →</Link><Link className="hero-link" href={`/basketball/source-stats/?q=${encodeURIComponent(name)}`}>Search stat fields →</Link></div></div>
