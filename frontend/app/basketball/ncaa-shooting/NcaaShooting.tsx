...
export default function NcaaShooting() {
  ...
  const download = () => {
    if (!result) return;
    downloadCsv(
      `ncaa-shooting-${season}-${metric}-page-${page + 1}.csv`,
      toCsv(
        ["Season", "Metric", "Minimum attempts", "Player", "NCAA player ID", "Program", "NCAA team ID", "Attempts", "Makes", "Points", "Average distance", "FG%", "3P%", "Rim%", "Midrange%", "Raw source stats JSON"],
        result.rows.map((row) => {
