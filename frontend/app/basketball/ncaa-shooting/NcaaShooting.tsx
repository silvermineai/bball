...
type Row = { season: number; player_id: string; team_id: string; player_name: string | null; team_name: string | null; value: number; stats: { attempts: number; makes: number; points: number; distance_sum: number; distance_count: number; zones: Record<string, Zone>; identity_basis?: string } };
...
export default function NcaaShooting() {
  ...
  useEffect(() => { fetch(`/api/basketball/research/ncaa-shooting?meta=1&season=${season}`).then((r) => { if (!r.ok) throw Error("The NCAA shooting catalog could not be loaded."); return r.json() as Promise<Meta>; }).then(setMeta).catch((e) => setError(e.message)); }, [retryNonce, season]);
