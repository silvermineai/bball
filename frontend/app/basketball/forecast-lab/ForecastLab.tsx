...
import { completeForecastLabMarketQuotes, forecastLabQuoteIdentity, hasCompleteForecastLabMarket, latestForecastLabMarketQuote } from "../../_lib/forecast-lab-market";
...
function marketIdentity(comparisons: Comparison[], market: Comparison["market"]) {
  const quote = marketQuote(comparisons, market);
  return quote ? forecastLabQuoteIdentity(quote) : null;
}
  ...
  const exportRows = () => downloadCsv(
    ...
    toCsv(
      ...
      rows.map((row) => [
        ...
        marketIdentity(row.comparisons, "spreads")?.observationId,
        marketIdentity(row.comparisons, "spreads")?.marketGameId,
        ...
        marketIdentity(row.comparisons, "totals")?.observationId,
        marketIdentity(row.comparisons, "totals")?.marketGameId,
        ...
        marketIdentity(row.comparisons, "h2h")?.observationId,
        marketIdentity(row.comparisons, "h2h")?.marketGameId,
