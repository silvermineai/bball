...
const exportRow = (result: Result, row: Row) => {
  ...
  return [result.season, labels[result.metric], row.rank, percentile(row.rank, result.total).toFixed(1), row.player_name, row.player_id, row.team_name, row.team_id, row.position, row.class_year, row.roster_height, row.hometown, row.high_school, row.games, row.minutes, row.points, row.rebounds, row.offensive_rebounds, row.defensive_rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.possessions, row.team_possessions, row.fgm, row.fga, row.tpm, row.tpa, row.ftm, row.fta, row.rim_makes, row.rim_attempts, row.mid_makes, row.mid_attempts, row.transition_points, row.unassisted_points, row.half_points, row.half_fga, row.half_fta, row.value, makes, attempts, row.component_count == null ? null : `${row.component_count}/8`, row.ppg_value, row.rpg_value, row.apg_value, row.spg_value, row.bpg_value, row.ts_value, row.ts_denominator, row.efg_value, row.efg_denominator, row.per40_value, row.rapm_net, row.per40_value, row.orapm, row.drapm, row.off_poss, row.def_poss];
};
...
export default function NcaaRankings() {
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>RANK BY</span><select name="ncaa-ranking-metric" value={metric} onChange={(e) => reset(() => setMetric(e.target.value as Metric))}>{(meta?.metrics || ["ppg", "rpg", "orpg", "drpg", "apg", "spg", "bpg", "fpg", "mpg", "topg", "ts", "efg", "half_ts", "per40", "ast_to", "stocks40", "tov_rate", "three_rate", "three_pct", "two_pct", "ft_pct", "rim_pct", "mid_pct", "ft_rate", "ast_rate", "points_poss", "orb40", "drb40", "reb40", "poss_share", "rim_rate", "transition_share", "unassisted_share", "rapm_net", "orapm", "drapm", "balanced_index", "impact_index"]).map((m) => <option key={m} value={m}>{labels[m]}</option>)}</select></label>
