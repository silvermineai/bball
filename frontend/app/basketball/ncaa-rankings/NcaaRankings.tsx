...
const exportRow = (result: Result, row: Row) => {
  ...
  return [result.season, labels[result.metric], row.rank, percentile(row.rank, result.total).toFixed(1), row.player_name, row.player_id, row.team_name, row.team_id, row.position, row.class_year, row.roster_height, row.hometown, row.high_school, row.games, row.minutes, row.points, row.rebounds, row.offensive_rebounds, row.defensive_rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.possessions, row.team_possessions, row.fgm, row.fga, row.tpm, row.tpa, row.ftm, row.fta, row.rim_makes, row.rim_attempts, row.mid_makes, row.mid_attempts, row.putback_makes, row.putback_attempts, row.transition_points, row.unassisted_total_attempts, row.unassisted_attempts, row.unassisted_points, row.half_points, row.half_fga, row.half_fta, row.value, makes, attempts, row.component_count == null ? null : `${row.component_count}/8`, row.ppg_value, row.rpg_value, row.apg_value, row.spg_value, row.bpg_value, row.ts_value, row.ts_denominator, row.efg_value, row.efg_denominator, row.per40_value, row.rapm_net, row.per40_value, row.orapm, row.drapm, row.off_poss, row.def_poss];
};
...
export default function NcaaRankings() {
  ...
  return <>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveRankings}>Retry live rankings</button></div> : !result ? <p className="empty" role="status">Loading rankings…</p> : <>
      ...
      <p className="note" style={{ marginTop: 24 }}>Retained player box and lineup editions. Net RAPM is available only where the exact archive ID appears in both releases; no name-only identity join is used. Rankings are descriptive archive statistics, not eligibility, recruiting grades or an outside identity match.</p>
    </>}
  </>;
}
