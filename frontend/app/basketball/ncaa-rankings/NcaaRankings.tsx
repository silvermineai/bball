...
type Metric = "ppg" | "rpg" | "apg" | "spg" | "bpg" | "ts" | "efg" | "per40" | "rapm_net" | "balanced_index" | "impact_index";
...
const labels: Record<Metric, string> = { ppg: "Points per game", rpg: "Rebounds per game", apg: "Assists per game", spg: "Steals per game", bpg: "Blocks per game", ts: "True shooting %", efg: "Effective FG %", per40: "Points per 40 minutes", rapm_net: "Net RAPM", balanced_index: "Balanced production index", impact_index: "Impact + production index" };
...
export default function NcaaRankings() {
  ...
  const download = () => {
    ...
    downloadCsv(
      ...
      toCsv(
        ["Season", "Metric", "Rank", "Player", "NCAA player ID", "Program", "NCAA team ID", "Position", "Class", "Games", "Minutes", "Points", "Rebounds", "Assists", "Steals", "Blocks", "Value", "Points per 40", "ORAPM", "DRAPM", "Net RAPM", "Offensive possessions", "Defensive possessions"],
        result.rows.map((row) => [result.season, labels[result.metric], row.rank, row.player_name, row.player_id, row.team_name, row.team_id, row.position, row.class_year, row.games, row.minutes, row.points, row.rebounds, row.assists, row.steals, row.blocks, row.value, row.per40_value, row.orapm, row.drapm, row.rapm_net, row.off_poss, row.def_poss]),
      ),
    );
  };
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>RANK BY</span><select value={metric} onChange={(e) => reset(() => setMetric(e.target.value as Metric))}>{(meta?.metrics || ["ppg", "rpg", "apg", "spg", "bpg", "ts", "efg", "per40", "rapm_net", "balanced_index", "impact_index"]).map((m) => <option key={m} value={m}>{labels[m]}</option>)}</select></label>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !result ? <p className="empty" role="status">Loading NCAA rankings…</p> : <>
      ...
      {result.metric === "rapm_net" && <p className="note">Net RAPM is the publisher&apos;s lineup estimate and is shown only when the exact NCAA player ID has an impact row. The source qualification is at least 500 offensive and 500 defensive possessions; the controls above additionally require the selected box-score games and minutes.</p>}
