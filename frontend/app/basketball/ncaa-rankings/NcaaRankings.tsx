...
export default function NcaaRankings() {
  ...
  const downloadManifest = () => {
    ...
    const manifest = {
      ...
      notes: ["Rows are descriptive archive statistics.", "Exact archive IDs are retained; no name-only join to outside identities is used.", "A source digest identifies the retained release and does not imply live roster status."],
    };
  ...
  return <>
    ...
    {meta?.sources?.length ? <details className="note" style={{ marginTop: 16 }}><summary>Edition receipts for {label(Number(season))}</summary><div className="table-scroll" style={{ marginTop: 12 }}><table className="data-table"><thead><tr><th>Dataset</th><th>Fetched (UTC)</th><th>SHA-256</th><th>Status</th></tr></thead><tbody>{meta.sources.map((source) => <tr key={source.dataset}><th>{source.dataset.replace(/^ncaa_/, "NCAA ")}</th><td>{sourceDate(source.fetched_at)}</td><td><code>{source.sha256 || "digest unavailable"}</code></td><td>Retained</td></tr>)}</tbody></table></div><div className="button-row" style={{ marginTop: 12 }}><button className="button secondary" type="button" onClick={downloadManifest}>Download edition manifest ↓</button></div><p style={{ marginTop: 12 }}>These receipts identify the retained editions behind this board. They describe the source archive, not live roster status.</p></details> : null}
