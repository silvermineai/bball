...
export default function NcaaRankings() {
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !result ? <p className="empty" role="status">Loading NCAA rankings…</p> : <>
      ...
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player</th><th>Program</th><th>Position</th><th>Class</th><th className="numeric">GP</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">{labels[result.metric]}</th></tr></thead><tbody>{result.rows.map((row) => <tr key={`${row.player_id}-${row.team_id}`}><td className="numeric"><strong>#{row.rank}</strong></td><td><Link href={`/basketball/ncaa-player-box/?season=${row.season}&q=${encodeURIComponent(row.player_id)}`}>{row.player_name || row.player_id} →</Link><small>NCAA player {row.player_id}</small><small><a href={`https://stats.ncaa.org/players/${encodeURIComponent(row.player_id)}`} target="_blank" rel="noreferrer">NCAA source ↗</a></small></td><td><strong>{row.team_name || row.team_id}</strong><small>NCAA team {row.team_id}</small></td><td>{row.position || "—"}</td><td>{row.class_year || "—"}</td><td className="numeric">{fmt(row.games, 0)}</td><td className="numeric">{fmt(row.minutes, 0)}</td><td className="numeric">{fmt(row.points, 0)}</td><td className="numeric">{fmt(row.rebounds, 0)}</td><td className="numeric">{fmt(row.assists, 0)}</td><td className="numeric"><strong>{fmt(row.value, result.metric === "ts" || result.metric === "efg" ? 1 : 2)}{result.metric === "ts" || result.metric === "efg" ? "%" : ""}</strong></td></tr>)}</tbody></table></div>
