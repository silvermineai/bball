...
type Metric = "ppg" | "rpg" | "apg" | "spg" | "bpg" | "fpg" | "mpg" | "topg" | "ts" | "efg" | "per40" | "ast_to" | "stocks40" | "tov_rate" | "three_rate" | "three_pct" | "ft_rate" | "ast_rate" | "points_poss" | "orb40" | "drb40" | "reb40" | "poss_share" | "rim_rate" | "transition_share" | "unassisted_share" | "rapm_net" | "orapm" | "drapm" | "balanced_index" | "impact_index";
...
export default function NcaaRankings() {
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>RANK BY</span><select value={metric} onChange={(e) => reset(() => setMetric(e.target.value as Metric))}>{(meta?.metrics || ["ppg", "rpg", "apg", "spg", "bpg", "fpg", "mpg", "topg", "ts", "efg", "per40", "ast_to", "stocks40", "tov_rate", "three_rate", "three_pct", "ft_rate", "ast_rate", "points_poss", "orb40", "drb40", "reb40", "poss_share", "rim_rate", "transition_share", "unassisted_share", "rapm_net", "orapm", "drapm", "balanced_index", "impact_index"]).map((m) => <option key={m} value={m}>{labels[m]}</option>)}</select></label>
