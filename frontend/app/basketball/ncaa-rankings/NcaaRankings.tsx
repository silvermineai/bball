...
export default function NcaaRankings() {
  ...
  return <>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveRankings}>Retry live rankings</button></div> : !result ? <p className="empty" role="status">Loading NCAA rankings…</p> : <>
      ...
      {(result.metric === "fpg" || result.metric === "topg" || result.metric === "mpg") && <p className="note">{result.metric === "fpg" ? "Fouls per game divides NCAA-recorded personal fouls by games played. It is a descriptive contact and availability signal; it is not a discipline grade or forecast input. Rows remain unavailable when the retained source lacks personal fouls." : result.metric === "topg" ? "Turnovers per game divides recorded turnovers by games played. It is a workload-dependent ball-security context signal; pair it with turnover rate and possessions before making a comparison." : "Minutes per game divides recorded minutes by games played. It describes workload, not a projected role or availability."}</p>}
