...
type Metric = "ppg" | "rpg" | "apg" | "spg" | "bpg" | "fpg" | "mpg" | "topg" | "ts" | "efg" | "per40" | "ast_to" | "stocks40" | "tov_rate" | "three_rate" | "three_pct" | "ft_pct" | "rim_pct" | "mid_pct" | "ft_rate" | "ast_rate" | "points_poss" | "orb40" | "drb40" | "reb40" | "poss_share" | "rim_rate" | "transition_share" | "unassisted_share" | "rapm_net" | "orapm" | "drapm" | "balanced_index" | "impact_index";
type Row = { season: number; player_id: string; team_id: string; player_name: string | null; team_name: string | null; position: string | null; class_year: string | null; games: number; minutes: number; points: number; rebounds: number; assists: number; steals: number; blocks: number; turnovers: number | null; fouls: number | null; fta?: number | null; ftm?: number | null; rim_attempts?: number | null; rim_makes?: number | null; mid_attempts?: number | null; mid_makes?: number | null; value: number; component_count?: number; ppg_value?: number | null; rpg_value?: number | null; apg_value?: number | null; spg_value?: number | null; bpg_value?: number | null; ts_value?: number | null; ts_denominator?: number | null; efg_value?: number | null; efg_denominator?: number | null; per40_value?: number | null; rank: number; rapm_net: number | null; orapm: number | null; drapm: number | null; off_poss: number | null; def_poss: number | null };
...
const exportRow = (result: Result, row: Row) => {
  ...
  return [result.season, labels[result.metric], row.rank, percentile(row.rank, result.total).toFixed(1), row.player_name, row.player_id, `https://stats.ncaa.org/players/${encodeURIComponent(row.player_id)}`, row.team_name, row.team_id, `https://stats.ncaa.org/teams/${encodeURIComponent(row.team_id)}`, row.position, row.class_year, row.games, row.minutes, row.points, row.rebounds, row.assists, row.steals, row.blocks, row.turnovers, row.fouls, row.value, makes, attempts, row.component_count == null ? null : `${row.component_count}/8`, row.ppg_value, row.rpg_value, row.apg_value, row.spg_value, row.bpg_value, row.ts_value, row.ts_denominator, row.efg_value, row.efg_denominator, row.per40_value, row.rapm_net, row.per40_value, row.orapm, row.drapm, row.off_poss, row.def_poss];
};
...
export default function NcaaRankings() {
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>RANK BY</span><select value={metric} onChange={(e) => reset(() => setMetric(e.target.value as Metric))}>{(meta?.metrics || ["ppg", "rpg", "apg", "spg", "bpg", "fpg", "mpg", "topg", "ts", "efg", "per40", "ast_to", "stocks40", "tov_rate", "three_rate", "three_pct", "ft_pct", "rim_pct", "mid_pct", "ft_rate", "ast_rate", "points_poss", "orb40", "drb40", "reb40", "poss_share", "rim_rate", "transition_share", "unassisted_share", "rapm_net", "orapm", "drapm", "balanced_index", "impact_index"]).map((m) => <option key={m} value={m}>{labels[m]}</option>)}</select></label>
