...
export default function NcaaRankings() {
  ...
  const download = () => {
    ...
    downloadCsv(
      ...
      toCsv(
        ["Season", "Metric", "Rank", "Player", "NCAA player ID", "Program", "NCAA team ID", "Position", "Class", "Games", "Minutes", "Points", "Rebounds", "Assists", "Steals", "Blocks", "Value", "Balanced components available", "Balanced PPG", "Balanced RPG", "Balanced APG", "Balanced SPG", "Balanced BPG", "Balanced TS %", "Balanced TS denominator", "Balanced eFG %", "Balanced eFG denominator", "Balanced P40", "Impact RAPM net", "Impact P40", "Impact ORAPM", "Impact DRAPM", "Impact offensive possessions", "Impact defensive possessions"],
        result.rows.map((row) => [result.season, labels[result.metric], row.rank, row.player_name, row.player_id, row.team_name, row.team_id, row.position, row.class_year, row.games, row.minutes, row.points, row.rebounds, row.assists, row.steals, row.blocks, row.value, row.component_count == null ? null : `${row.component_count}/8`, row.ppg_value, row.rpg_value, row.apg_value, row.spg_value, row.bpg_value, row.ts_value, row.ts_denominator, row.efg_value, row.efg_denominator, row.per40_value, row.rapm_net, row.per40_value, row.orapm, row.drapm, row.off_poss, row.def_poss]),
      ),
    );
  };
  return <>
