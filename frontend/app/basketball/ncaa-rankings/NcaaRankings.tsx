...
type Metric = "ppg" | "rpg" | "orpg" | "drpg" | "apg" | "spg" | "bpg" | "fpg" | "mpg" | "topg" | "ts" | "efg" | "half_ts" | "per40" | "ast_to" | "stocks40" | "tov_rate" | "three_rate" | "three_pct" | "ft_pct" | "rim_pct" | "mid_pct" | "ft_rate" | "ast_rate" | "points_poss" | "orb40" | "drb40" | "reb40" | "poss_share" | "rim_rate" | "transition_share" | "unassisted_share" | "rapm_net" | "orapm" | "drapm" | "balanced_index" | "impact_index";
...
const accuracySample = (metric: Metric, row: Row) => {
...
export default function NcaaRankings() {
  ...
  return <>
    ...
    <div className="toolbar">
      ...
      <label className="control"><span>RANK BY</span><select name="ncaa-ranking-metric" value={metric} onChange={(e) => reset(() => setMetric(e.target.value as Metric))}>{(meta?.metrics || ["ppg", "rpg", "orpg", "drpg", "apg", "spg", "bpg", "fpg", "mpg", "topg", "ts", "efg", "half_ts", "per40", "ast_to", "stocks40", "tov_rate", "three_rate", "three_pct", "ft_pct", "rim_pct", "mid_pct", "ft_rate", "ast_rate", "points_poss", "orb40", "drb40", "reb40", "poss_share", "rim_rate", "transition_share", "unassisted_share", "rapm_net", "orapm", "drapm", "balanced_index", "impact_index"]).map((m) => <option key={m} value={m}>{labels[m]}</option>)}</select></label>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveRankings}>Retry live rankings</button></div> : !result ? <p className="empty" role="status">Loading rankings…</p> : <>
      ...
      {(result.metric === "ft_pct" || result.metric === "rim_pct" || result.metric === "mid_pct") && <p className="note">{result.metric === "ft_pct" ? "Free-throw accuracy divides recorded free throws made by free throws attempted." : result.metric === "rim_pct" ? "Rim accuracy divides recorded rim makes by rim attempts." : "Midrange accuracy divides recorded midrange makes by midrange attempts."} Each board uses its matching attempt denominator and keeps rows unavailable when the retained archive lacks that denominator. These are descriptive recorded rates, not forecast inputs.</p>}
