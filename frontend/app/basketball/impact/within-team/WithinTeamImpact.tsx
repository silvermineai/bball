...
export default function WithinTeamImpact() {
  ...
  return <>
    <div className="dateline eyebrow"><span>Player impact / within-team RAPM</span><span>{catalog?.seasons.length ?? 17} seasons · derived</span></div>
    <div className="page-title"><div className="eyebrow">The within-team impact archive</div><h1>Measure impact<br /><em>in the team context.</em></h1><p>Read within-team adjusted plus-minus alongside playing volume and roster size. It answers a different question from league-wide RAPM: which players moved their team&apos;s results relative to the teammates in the same model?</p><div className="hero-actions"><a className="hero-link" href="/basketball/impact/">Open league-wide RAPM →</a></div></div>
    ...
    <section className="section">
      <div className="section-heading"><div><div className="eyebrow">Team impact finder</div><h2>Compare players within their team.</h2></div><span className="note">{seasonLabel(season)} · one row per player/team</span></div>
      <div className="toolbar">
        ...
        <label className="control"><span>PLAYER OR TEAM</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search names or teams" /></label>
        ...
        <a className="button secondary" href={`/api/basketball/research/impact-within-team/source?season=${encodeURIComponent(String(season))}`}>Download team-impact archive ↓</a>
      </div>
      ...
      {error ? <p role="alert" className="status-error">{error}</p> : !edition ? <p role="status" className="empty">Loading within-team impact rows…</p> : <>
        ...
        <div className="table-scroll"><table className="data-table"><thead><tr><th>Rank</th><th>Player / archive identity</th><th>Team</th><th className="numeric">ORAPM</th><th className="numeric">DRAPM</th><th className="numeric">Net</th><th className="numeric">Team poss.</th><th className="numeric">Model players</th></tr></thead><tbody>{visible.map((row: WithinImpactRow) => <tr key={`${row.season}-${row.player_id}-${row.team_id}`}><td className="rank-number">{row.rank ?? "—"}</td><td><strong>{row.player}</strong><small>{row.player_code}</small><small>Archive ID {row.player_id}</small></td><td>{row.team}</td><td className="numeric">{row.rapm_off == null ? "—" : signed(row.rapm_off)}</td><td className="numeric">{row.rapm_def == null ? "—" : signed(row.rapm_def)}</td><td className="numeric">{row.rapm_net == null ? "—" : signed(row.rapm_net)}</td><td className="numeric">{fmt(row.team_off_poss, 0)}</td><td className="numeric">{fmt(row.num_players, 0)}</td></tr>)}</tbody></table></div>
        {!visible.length && <p className="empty">No impact rows match these filters.</p>}
      </>}
    </section>
    <section className="section paper-panel"><h2>How to read this measure.</h2><p>Within-team RAPM is a regularized plus-minus estimate. The edition supplies team offensive possessions as the volume field and does not provide a separate defensive-possession column, so the 500-possession filter is defined on the available team sample. A positive net value is descriptive evidence within that team-season; it is not a transfer grade, a forecast feature or a claim that a player would have the same value in a different program.</p><p>Player codes and team IDs remain in the archive namespace. This board is separate from the league-wide RAPM board and the Silvermine forecast.</p></section>
  </>;
}
