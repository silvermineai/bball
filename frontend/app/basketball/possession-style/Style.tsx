...
export default function Style({ catalog }: { catalog: PossessionStyleCatalog }) {
  ...
  return <>
    ...
    <p className="note" role="status">{status === "live" ? "Cloudflare D1 possession-style archive connected; rows are aggregated from the retained dataset edition." : status === "fallback" ? <>Cloudflare D1 archive unavailable; showing the verified bundled edition. <button className="button secondary" type="button" onClick={() => { setError(""); setStatus("checking"); setRetryNonce((value) => value + 1); }}>Retry live archive</button></> : "Checking the Cloudflare D1 archive…"}</p>
    ...
    {source && <p className="note"><strong>Edition receipt:</strong> fetched {source.fetched_at ? date(source.fetched_at) : "—"} · SHA-256 {source.sha256?.slice(0, 16) || "—"}… · retained archive</p>}
