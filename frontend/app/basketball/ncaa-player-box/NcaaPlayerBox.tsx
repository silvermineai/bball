...
export default function NcaaPlayerBox() {
  ...
  return <>
    ...
    {error ? <p className="status-error" role="alert">{error}</p> : !result ? <p className="empty" role="status">Loading NCAA player rows…</p> : <>
      <p className="note">{result.total.toLocaleString()} matching game rows · page {page + 1} of {pages} · points, minutes, rebounds, assists and shooting splits come from the source release.</p>
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Date / player</th><th>Matchup</th><th className="numeric">MIN</th><th className="numeric">PTS</th><th className="numeric">REB</th><th className="numeric">AST</th><th className="numeric">FG</th><th className="numeric">3P</th><th className="numeric">TS%</th></tr></thead><tbody>{result.rows.map((row) => { const s = row.stats; return <tr key={`${row.contest_id}-${row.team_id}-${row.player_id}`}><td><strong>{row.player_name || row.player_id}</strong><small>{row.game_date || "—"} · NCAA player {row.player_id}</small></td><td><strong>{row.team_name || row.team_id}</strong><small>vs {row.opponent_name || "—"} · contest {row.contest_id}</small></td><td className="numeric">{n(s.mins)}</td><td className="numeric"><strong>{n(s.pts, 0)}</strong></td><td className="numeric">{n((s.orb || 0) + (s.drb || 0), 0)}</td><td className="numeric">{n(s.ast, 0)}</td><td className="numeric">{pct(s.fg_pct)}</td><td className="numeric">{pct(s.tp_pct)}</td><td className="numeric">{pct(s.ts_pct)}</td></tr>; })}</tbody></table></div>
