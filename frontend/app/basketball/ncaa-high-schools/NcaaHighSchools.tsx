...
export default function NcaaHighSchools() {
  ...
  return <>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveArchive}>Retry high-school pipeline</button></div> : !result ? <p className="empty" role="status">Loading high-school pipeline…</p> : <>
      ...
      <p className="note" style={{ marginTop: 24 }}>Source: retained team roster release via the retained archive{meta?.source?.fetched_at ? ` · receipt fetched ${new Date(meta.source.fetched_at).toLocaleDateString()}` : ""}. The clock describes the retained source edition, not a live roster, eligibility, or transfer update. High-school labels are retained as supplied and may reflect spelling or campus naming differences; the aggregation does not assert a recruiting relationship, commitment, or athlete identity beyond the source row.</p>
    </>}
  </>;
}
