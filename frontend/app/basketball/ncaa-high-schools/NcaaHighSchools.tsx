...
type Meta = { seasons: number[]; total: number; metrics: Metric[]; source?: { fetched_at?: string | null; sha256?: string | null } };
...
export default function NcaaHighSchools() {
  ...
  const [metric, setMetric] = useState<Metric>(initialMetric && Object.prototype.hasOwnProperty.call(labels, initialMetric) ? initialMetric as Metric : "players");
  const [minPlayers, setMinPlayers] = useState(initial?.get("minPlayers") || "1");
  ...
  const share = async () => {
    try {
      ...
      setCopied("Pipeline link copied.");
    } catch {
  ...
  return <>
    ...
    {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveArchive}>Retry high-school pipeline</button></div> : !result ? <p className="empty" role="status">Loading high-school pipeline…</p> : <>
      ...
      <p className="note" style={{ marginTop: 24 }}>Data note: this is a retained roster edition{meta?.source?.fetched_at ? ` · captured ${new Date(meta.source.fetched_at).toLocaleDateString()}` : ""}. The clock describes the edition, not a live roster, eligibility, or transfer update. High-school labels are retained as supplied and may reflect spelling or campus naming differences; the aggregation does not assert a recruiting relationship, commitment, or athlete identity beyond the recorded row.</p>
    </>}
  </>;
}
