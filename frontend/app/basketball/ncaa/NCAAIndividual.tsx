  ...
  return (
    <>
      ...
      {error ? <p role="alert" className="status-error">{error}</p> : !data ? <p role="status" className="empty">Loading NCAA national records…</p> : <>
        ...
        <div className="section-heading" style={{ marginBottom: 20 }}><p>{rows.length.toLocaleString()} matching records · {ncaaStatLabels[stat]}</p><button className="button secondary" type="button" onClick={download}>Download CSV ↓</button></div>
