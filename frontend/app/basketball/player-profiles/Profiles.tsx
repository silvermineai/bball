...
import { playerProfileStatHref } from "../../_lib/player-profile-links";
  ...
  return (
    <>
      ...
      {error ? <div className="status-error" role="alert"><span>{error}</span><button className="button secondary" type="button" onClick={retryLiveArchive}>Retry player archive</button></div> : !result ? <p className="empty" role="status">Loading player archive…</p> : <>
        ...
        <div className="table-scroll"><table className="data-table"><thead><tr><th>Player</th><th>Season</th><th>Program</th><th>Position</th><th>Size</th><th>Jersey</th><th>Experience</th><th>Status</th><th>Archive ID</th><th>Production</th></tr></thead><tbody>{result.rows.map((r) => { const statHref = playerProfileStatHref(r.id, r.season); return <tr key={`${r.season}-${r.id}`}><td><Link href={`/basketball/player/?id=${encodeURIComponent(r.id)}&season=${r.season}`}>{r.name || r.id} →</Link><small><Link href={`/basketball/recruiting/?q=${encodeURIComponent(r.name || r.id)}`}>Search dated evidence →</Link></small></td><td>{label(r.season)}</td><td>{r.team_id ? <Link href={`/basketball/programs/${encodeURIComponent(r.team_id)}/`}>{r.team || r.team_id}</Link> : (r.team || "—")}<small>{r.team_id ? `Team archive ID ${r.team_id}` : "Team unavailable"}</small></td><td>{r.position || "—"}</td><td>{[r.height, r.weight].filter(Boolean).join(" · ") || "—"}</td><td className="numeric">{r.jersey || "—"}</td><td className="numeric">{r.experience || "—"}</td><td>{r.status || "—"}</td><td><small>{r.id}</small></td><td>{statHref ? <Link href={statHref}>Open exact-ID stats →</Link> : <span className="note">Unavailable for this ID</span>}</td></tr>; })}</tbody></table></div>
