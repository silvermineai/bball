...
export default function PbpArchive({
  ...
  return (
    <section className="section">
      ...
      <div className="table-scroll"><table className="data-table"><thead><tr><th>Date</th><th>Matchup</th><th className="numeric">Events</th><th className="numeric">Scoring</th><th className="numeric">Shots</th><th>Source</th></tr></thead><tbody>{visible.map((game: PbpGame) => <tr key={(active?.season || "") + "-" + game.id}><td>{game.date ? date(game.date) : "—"}</td><td><strong>{game.away || "Away"} at {game.home || "Home"}</strong><small>ESPN game {game.id}</small></td><td className="numeric">{fmt(game.events, 0)}</td><td className="numeric">{fmt(game.scoring_plays, 0)}</td><td className="numeric">{fmt(game.shooting_plays, 0)}<small>{fmt(game.shot_attempts, 0)} accepted shot attempts</small></td><td><a href={sourceUrl(game.id)} target="_blank" rel="noreferrer">Open ESPN ↗</a></td></tr>)}</tbody></table></div>
