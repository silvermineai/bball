...
def dataset_rows(overview: dict) -> dict[str, int]:
    ...
    expected = {
        ...
        # D1 keeps the eight most recent game-level NCAA releases for fast
    ...
    with sqlite3.connect(local_path) as database:
        expected["bb_ncaa_player_box"] = int(database.execute(
            "SELECT COUNT(*) FROM bb_ncaa_player_box WHERE season IN (2019, 2020, 2021, 2022, 2023, 2024, 2025, 2026)"
        ).fetchone()[0])
