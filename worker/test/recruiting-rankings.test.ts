  ...
  it("returns source-labeled ranked prospects with parsed school IDs", async () => {
    const prepare = vi.fn((sql: string) => ({
      bind: vi.fn(() => ({
        ...
        all: vi.fn(async () => ({ results: sql.includes("SELECT TRIM(r.committed_team_name)") ? [{ team: "PG", total: 1, ranked_total: 1, top100_total: 1, best_rank: 1, average_rank: 1 }] : sql.includes("GROUP BY") ? [{ position: "PG", total: 1 }] : [{ athlete_id: "272415", name: "Danny Abass", rank: 225, school_ids_json: '["257","526"]' }] })),
      })),
    }));
    const response = await recruitingRankings.request("/?season=2027&page=0", {}, { RESEARCH_DB: { prepare } });
    expect(response.status).toBe(200);
    const body = await response.json() as { total: number; cohort: { committed: number; ranked: number; graded: number }; position_breakdown: Array<{ position: string; total: number }>; commitment_destinations: Array<{ team: string; total: number; ranked_total: number; top100_total: number; best_rank: number | null; average_rank: number | null }>; rows: Array<{ name: string; school_ids: string[]; school_ids_json?: string }> };
    expect(body.total).toBe(1);
    expect(body.cohort).toEqual({ committed: 1, ranked: 1, graded: 1 });
    expect(body.position_breakdown).toEqual([{ position: "PG", total: 1 }]);
    expect(body.commitment_destinations).toEqual([{ team: "PG", total: 1, ranked_total: 1, top100_total: 1, best_rank: 1, average_rank: 1 }]);
