...
describe("bball api", () => {
  ...
  it("returns a compact exact-ID football career trail", async () => {
    ...
    expect(response.status).toBe(200);
    const body = await response.json() as { player_id: string; seasons: number[]; source_records: number; box_records: number; box_games: Record<string, number>; box_totals: Array<Record<string, unknown>>; rows: Array<Record<string, unknown>> };
    expect(body).toMatchObject({ player_id: "123", seasons: [2025, 2024], source_records: 2, box_records: 1, box_games: { "2025": 1 } });
  ...
  it("serves D1-backed basketball forecasts with bounded filters and parsed predictions", async () => {
    ...
    const body = (await response.json()) as {
      ...
      page_size: number;
      total: number;
      rows: Array<{ prediction: Record<string, unknown> | null; prediction_integrity: string; source_start?: string | null; source_time_valid?: boolean | null; source_observed_at?: string | null }>;
    };
    ...
    expect(body.rows[0].prediction_integrity).toBe("valid");
    ...
    expect(body.rows[1].prediction_integrity).toBe("invalid");
