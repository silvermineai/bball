...
ncaaPlayerCard.get("/:id/games", zValidator("query", gamesQuerySchema), async (c) => {
  ...
  const [count, result] = await gameDb.batch([
...
ncaaPlayerCard.get("/:id", zValidator("query", querySchema), async (c) => {
  ...
  const [seasons, rosters, shooting] = await researchDb(c.env).batch([
  ...
  const games = await ncaaBoxDb(c.env).prepare("SELECT season,contest_id,team_id,game_date,team_name,opponent_name,player_name,stats_json FROM bb_ncaa_player_box WHERE player_id=? AND season=? ORDER BY game_date DESC,contest_id DESC LIMIT 12").bind(playerId, season).all();
