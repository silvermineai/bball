...
`/basketball/evaluation/` compares the published preseason model with a weekly updating ridge challenger and publishes three independent holdout transitions: 2023–24, 2024–25 and 2025–26. The basketball navigation, model notebook and journal link to it. This is an exploratory retrospective experiment, separate from the production 2026–27 forecasts and the prospective ledger.
...
The 2023–24 replay calibrates on 2022–23 predictions from cached schedule/team-box source editions, then freezes that mapping for the test season. The 2024–25 replay supplies 5,701 raw predictions for its two-parameter logistic calibration and 80th-percentile absolute-margin interval. The 2025–26 transition independently calibrates on 2024–25 and freezes that mapping for its test season. The main calibration rows are downloadable and explicitly labeled as calibration inputs, not independent test results; all three transitions publish their holdout rows and fit evidence in `transitions.json`.

The three transitions use 69 weekly holdout fits: 23 for each test season. The 2022 schedule/team-box editions are loaded from the local source cache for replay only; the 2023–26 production receipts remain the D1-backed evaluation source. Later source corrections can appear in historical records. The 2025–26 season had already been used for the published preseason evaluation; this is a new exploratory comparison on that season, not an untouched external test or a real-time record. No prospective forecasts or production model registrations are changed.
...
- `transitions.json`: a small index for the three dated holdouts. The linked `transition-2024.json`, `transition-2025.json` and `transition-2026.json` files keep each evidence bundle below the static-asset file limit while retaining 17,130 game rows, each frozen preseason model, calibration mapping, calibration fits and weekly holdout fits.
...
Six focused Python tests exercise source-release pairing, time-zone normalization, strict cutoffs, frozen membership, future-outcome perturbation, default-fit parity, metric arithmetic and paired resampling edge cases. All 41 basketball Python tests pass. The frontend evaluation tests independently recompute every published main comparison aggregate, conserve calibration-bin game counts, check filter cohorts/empty states, verify CSV evidence and require all three transition summaries. The complete frontend suite has 57 passing files and 213 tests; the Worker has 18 passing files and 106 tests and passes type checking.

The transition bundle exposes each holdout's exact training IDs, normalized-feature hashes, cutoffs and stored coefficients for independent replay; the main artifact still recomputes all 5,734 weekly forecasts directly from stored coefficients and preserves parity with the 1,579 existing production forecasts. Local and production browser QA pass desktop/mobile rendering, calibration interaction, monthly selection, venue/program filters, missing results, pagination, fit evidence and selected CSV identities. The eight live JSON files are covered by the experiment manifest and deployment build. The live journal, notebook links, sitemap, unchanged production overview and adjacent desks were also verified after deployment. The private R2 bundle passed its download-and-hash check.
