...
`/basketball/players/` searches annual production from 24 published box-score seasons, ending 2003 through 2026. `/basketball/player/?id=32284&season=2009` opens a source identity's historical record with season selection, program-level production, interactive development comparisons and complete retained game logs. The player index preserves season, search, sort, qualification and page controls in the URL and provides a copy-link action for sharing an exact ranking slice. Its season receipt panel shows the active source release URL, retrieval clock and SHA-256, and those same receipt fields travel with the filtered CSV export. Existing links from program dossiers, recruiting and shooting profiles continue to use the same route.
...
The archive has a standalone publisher; the main basketball publisher also rebuilds historical derivatives from the cached source files before building the site, then syncs changed historical editions before deployment. The separate historical warehouse does not feed extra seasons into the preseason model. The synchronizer reads actual remote season editions and skips a batch only when all seasons it contains already match. A fully current warehouse skips archive and SQL uploads. Annual index hashes are checked against the export manifest, and a changed local annual index is rebuilt from source rather than trusted solely by its edition label. The public player index carries source-record, DNP, excluded and publisher-reported start counts; starter rate is calculated only over rows with an explicit start flag. The current release also exposes source-recorded offensive rebounds, defensive rebounds and personal fouls per game, with matching sort controls and CSV columns. No recurring historical job is installed.

The `/basketball/crosswalk/` desk exposes the 2025–26 SportsDataverse player crosswalk in the research D1. It searches 5,442 source rows and keeps ESPN, Fox Sports and Yahoo identifiers, match method, confidence and the release receipt together. The crosswalk is a provider-key navigation aid: it does not join NCAA identifiers or establish eligibility, transfer status, roster availability or a unique person outside the publisher's own match evidence.
