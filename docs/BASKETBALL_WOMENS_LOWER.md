...
The target-season probe is retained separately for men’s and women’s D2/D3. It queried the NCAA persisted scoreboard contract for September through November 2026 with six response receipts per gender; the current responses contain zero calendar rows and zero contests. The UI reports that result as unavailable target-season evidence, never as an empty schedule, and the publication monitor checks the probe scope, receipts and freshness.
