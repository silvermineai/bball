...
Rebuild with `npm --prefix frontend run build`; run `npm --prefix frontend test`. A static-only deployment uses `scripts/cloudflare.py deploy` after validation. Completed-game archive retention and a recurring publication job remain separate unfinished work.
