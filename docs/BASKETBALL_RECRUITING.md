...
The publisher wire links to the retained ESPN and NCAA.com article feeds for context and does not copy article bodies, photographs or biographies. NCAA.com D-I, D-II and D-III feed scope is preserved and searchable on the news archive. It now supports search across all retained recruiting-context stories, transfer/prep/draft/eligibility topic filters, shareable URLs, pagination and CSV export. It is not a transaction ledger: headlines and descriptions do not establish eligibility, destination or availability. The board does not contain proprietary recruiting grades, stars, rankings, valuations or contact information. These events do not change preseason forecasts. Prep and international statistics remain outside the college box-score archive.

The announcement view includes a **review priority** order for the coaching desk. It places later non-addition statements first, then exact normalized roster handoffs, linked prior production, prior minutes and publication date. This is an evidence-triage order, not a recruiting grade; its criteria remain visible in the page note and the underlying filters are preserved in the URL.
...
The page also reports coverage for an **authorized recruiting intake**. An operator with a provider agreement can validate a dated transfer, commitment or eligibility export through [`RECRUITING_INTAKE.md`](RECRUITING_INTAKE.md). The importer requires a license URL, HTTPS source URL, stable provider record, source publication date and capture clock, and rejects the complete file if any row is malformed. D1 stores the raw audit payload separately; the public endpoint exposes only provider/row counts and capture clocks, so a licensed feed is not republished. These source-reported records remain separate from the curated announcements, roster observations and forecast inputs.
...
The recruiting fit board provides a separate five-player private shortlist for a selected program and shows role, transparent fit score, prior workload, production, efficiency and publisher Box BPM side by side. Its role room reports prior minutes as returning, incoming or explicitly unclassified when the source row is ambiguous; it never assigns residual minutes to a transfer or departure. It uses the same exact source IDs and prior-season fields as the fit ranking; missing values remain unavailable. Prior production also carries publisher-reported starts, the number of rows with a start flag, and the resulting reported starter rate; those fields describe the source sample and do not infer a future role. Each selected source ID can carry a browser-local note capped at 500 characters; notes stay out of the model and are included in the board CSV as working context.
...
The same workload table is available in the recorded 2025–26 and 2024–25 views across their observed programs. Those historical views use playing time on both sides of the exact-ID comparison, so they describe observed continuity and different-program records without claiming a transfer event, reason or eligibility outcome. The 2024–25 edition is the earliest roster snapshot currently retained in this source family.

## Official transfer context

The recruiting page links to the [NCAA transfer rules and eligibility page](https://www.ncaa.org/eligibility-center/transfer-rules-and-eligibility/) and the [NCAA Student-Athlete Transfer Research page](https://www.ncaa.org/what-we-do/research/student-athlete-transfer-research/), plus the NCAA-hosted Tableau views for [DI transfer composition](https://public.tableau.com/views/RES_Transfer_Dash_Final/Transfer_Dash_1), [DI portal trends](https://public.tableau.com/views/DI_Transfer_Portal_2024/2026DIupdate), and [DII portal trends](https://public.tableau.com/views/DII_Transfer_Portal_2024/2026DIIupdate). It also hands readers to the NCAA [men's player-availability portal](https://www.ncaa.com/di-mens-basketball-player-availability) and [published availability archive](https://www.ncaa.com/di-mens-basketball-player-archive) for tournament reporting. The NCAA describes the Transfer Portal as a centralized database, while its research dashboards provide aggregate transfer composition and portal context. The availability pages are an official application/archive and are linked for source verification; Silvermine does not mirror their protected application or infer an availability ruling from a roster observation. These official pages do not turn a Silvermine roster observation or school announcement into a player-level portal transaction or eligibility determination. The current edition therefore keeps player-level recruiting evidence source-linked and explicitly labels its coverage as partial.

The announcement view also includes an all-program coverage map. Each of the 354 source-listed programs is labeled either as having reviewed school announcements in this edition or as roster observation only, with announced-addition counts, linked prior profiles and workload context where available. The map can be sorted by unrepresented prior minutes to surface programs whose listed roster leaves the largest prior workload pool for film and source review. Each program also links directly into its filtered roster-observation rows. This makes partial coverage visible instead of allowing an unlisted program to look like a confirmed zero.
...
Roster observations also carry an optional `prior_production` summary when the exact source athlete ID has recorded prior-season statistics: games, minutes, minutes per game, counting rates, shooting efficiency, usage rates and prior programs. These are aggregated across prior program stints using recorded game counts; they describe historical workload only and never establish transfer status, eligibility or a new-school role. The roster board can sort by prior minutes, source-reported starter rate, points, rebounds, assists, steals, blocks, true shooting, effective field goal percentage, three-point accuracy, free-throw rate, three-point attempt rate, turnover rate (lower is favorable) or publisher Box BPM, and includes the full profile in its CSV export. Minimum prior-games, prior-minutes and reported starter-rate controls make sample quality explicit; a starter-rate threshold excludes rows where the source did not report a start flag. Observation filters include source-reported position and class labels, alongside program/player search and movement status. They are encoded in `rosterSeason`, `rosterQ`, `rosterPosition`, `rosterClass`, `rosterStatus`, `rosterSort`, `rosterMinGames`, `rosterMinMinutes`, `rosterMinStarterRate` and `rosterPage` query parameters so a staff can share the exact slice being reviewed.
