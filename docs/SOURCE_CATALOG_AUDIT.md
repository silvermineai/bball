...
The upstream pipeline documents additional `game_rosters` and `officials` outputs. Silvermine now retains the 2024–25 and 2025–26 releases in the dedicated NCAA archive with a separate provenance contract, searchable through the [game context desk](/basketball/game-context/). Starter, active, DNP and ejection flags remain game-day context rather than player statistics; officials remain a separate assignment table. They are not joined into the player-stat warehouse or forecast model.
