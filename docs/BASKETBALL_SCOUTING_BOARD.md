...
`season` selects the archive year, `w` stores eight raw weights in the metric order above, `q` stores search, `pos` stores the literal source position, `min` accepts 0/20/30 minutes per game, and repeated `pick` parameters retain up to three exact `season:player_id:team_id` identities. Season changes open that season's private browser watchlist when one exists; an explicit shared URL takes precedence. Filters and presets are shareable and support browser navigation. Copying an untouched board serializes its season and all default weights; clipboard failures expose a selectable link field. Text/range changes replace the current URL entry; discrete choices push an entry. Invalid settings produce a visible explanation, and unsupported seasons produce no board.
