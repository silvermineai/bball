...
The first stored edition contains 2,323 game snapshots: 744 football and 1,579 basketball. The deployed archive now retains a second revision for each of those games as well. It also stores the shared research ledger and the supporting football and basketball evidence files required by those pages. New deployments run the capture hook after the Worker deploy succeeds. If capture or verification fails, the deployment remains successful and the archiver can be retried with `scripts/archive-briefs.py` without redeploying. Current basketball brief HTML stays in Workers Assets so the live 2026–27 pages remain available even when the archive database is at capacity; the Worker falls back to the matching immutable R2 revision only when a current asset is absent. Football `/blog/game-*` pages continue to use the archive-first path. An asset-only deploy can explicitly skip the optional archive write with `SKIP_BRIEF_ARCHIVE=1` when D1 storage is full.
