...
The position continuity watch breaks the same prior-minute denominator into guard, forward and center groups. It surfaces the five lowest same-program returning shares for each group, alongside incoming prior minutes, so a staff can identify a role-specific recruiting question without turning a partial roster into a depth chart. A position is included only when prior minutes were observed for that source-labeled group.

The derived rows are built by `frontend/app/_lib/roster-readiness.ts` from the published roster and basketball overview editions. Tests cover exact workload denominators, missing prior production, position normalization, position-specific continuity and schedule coverage. CSV exports preserve the source IDs, counts, raw minute totals, roster shape, position workload shares, rating context and schedule counts.
