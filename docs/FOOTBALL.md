...
The live football archive contains 37,820 schedule records, 709,608 player box-score rows, 1,015,076 NCAA-derived player-game rows, 51,524 season-roster rows and 8,624 recruiting rows across the retained editions. Coverage spans multiple divisions. The production/live API model is FBS-vs-FBS. The receipt-backed matchup archive also publishes a separately trained exact-FCS static score cohort plus exact-division D2 and D3 score models when each division clears its dated-history gates; the FCS cohort stays separate from the primary API model so model lineage cannot be mixed. This is **not** a complete roster census, and source row counts are not unique-athlete counts.

The live edition uses a five-season schedule window and excludes older retained history from the production fit. The September 22, 2026 model edition covers 604 upcoming FBS forecasts from 2,610 current-season schedule rows; 2,006 rows are outside the trained FBS field. Its fixed 2025 holdout covers 784 games and excludes 24 games involving unseen teams. Probability-pick accuracy is 65.43%; margin-pick accuracy is 66.20%. Margin MAE is 14.24 points, margin RMSE 17.99 and total MAE 12.98. A constant home-margin baseline has 15.99-point MAE on the same games. These are retrospective baseline results, not proof of market advantage.

The current football scorecard keeps 604 rows for the active model edition. A September 21 capture retained 1,447 pregame market observations in the live archive and linked 49 upcoming games to at least one exact-ID spread, total or moneyline comparison. These are capture-clock observations from ESPN Summary pickcenter responses, not verified closing lines.

The same edition retains 1,015,076 NCAA-derived player game rows across 2013–2025 (53,479 in 2013, 53,994 in 2014, 52,049 in 2015, 52,461 in 2016, 52,610 in 2017, 53,258 in 2018, 55,330 in 2019, 28,405 in 2020, 53,545 in 2021, 56,840 in 2022, 236,492 in 2023, 59,781 in 2024 and 206,832 in 2025). These records expand source evidence and search coverage; their names are not stable athlete identities and they do not enter the FBS forecast fit.
...
- The separate `ncaa_mfb_player_stats` release supplies NCAA-derived player game rows for 2013–2025. Silvermine retains its full source fields in the `ncaa_player_stats` namespace and uses the ESPN game ID when present for schedule context. The release has player names, team IDs and contest IDs but no stable athlete ID; those rows are searchable source evidence only and never join to ESPN player careers by name.
...
Production verification checked byte equality for the model/evidence/ledger assets, methodology, forecast guide, sample brief and homepage, and confirmed the basketball overview was unchanged. Live D1 history retains both v1 and v2 with their original registration clocks. Deployed Worker version: `160e4aaf-c504-4c02-a71e-62b3c6df33fb`.
...
The [NCAA player leaders board](/football/ncaa-leaders/) adds season-scoped aggregates from the complete NCAA-derived player-game release for passing, rushing, receiving, defense, kicking and punt returns. It sums the retained counting fields within source name, team ID and category, shows source-row/game denominators, and links back to raw records and the attributed release. Season archive pages cover every release with a supported leader category from 2020 onward; older NCAA rows remain in the raw archive when their source category is not one of the six leaderboard categories. The board deliberately excludes team aggregate rows and never merges names across seasons or teams because the release does not provide a stable athlete ID. These leaders are a source-native triage view, separate from the identified EPA rankings and forecast model.

The player board also shows an **EPA-per-play cohort percentile** beside the source or Silvermine rank. It compares finite observed rates within the selected season, division, category and qualification cohort, keeps ties together, excludes unavailable rates from the denominator and carries the value into CSV exports. It is a descriptive rate context for learning about workload and efficiency; it is not a composite grade, a replacement for the source rank or a forecast input.

`/api/football/players/:id?season=2010&page=0` returns an exact-athlete-ID season summary alongside up to 50 source rows with schedule context for the 17-season 2010–2026 archive. The summary exposes publisher passing, rushing and receiving aggregates (EPA, plays, yards, touchdowns, games and available EPA rank) plus counts of additional box-score categories; separate categories are never added into a composite. `/api/football/coverage` returns authoritative D1 row counts. Public pages read generated static artifacts; player logs and the football matchup forecast rows read D1 directly, with the matchup page retaining its static evidence fallback. The `/football/players/` board preserves season, category, division, search, qualification and page controls in the URL, so a coach can share a specific evaluation slice.
`/api/football/source-stats?dataset=box&season=2025&q=player&page=0` is the bounded raw-source browser for every retained football dataset: player boxes, passing/rushing/receiving aggregates, defensive and specialist events, NCAA-derived player game stats, advanced team rates, team directory rows and the historical market archive. It returns parsed source fields with exact record keys, optional game context and the matching source receipt URL, retrieval clock and SHA-256; `dataset=all` provides a cross-dataset view. Literal search, season, team, game and page filters are validated server-side. The `/football/source-stats/` page makes those records discoverable while preserving the name-only boundary for defense, specialist and NCAA-derived player releases. Its page export includes receipt columns, and its bounded full export walks every matching API page (up to 1,001 pages) with progress feedback; narrow the literal search when a slice exceeds that browser safety limit.
...
The legacy SportsDataverse football market archive contains **zero verified pregame observations** because its rows lack bookmaker publication time. The research ledger also captures prospective ESPN Summary pickcenter rows for future 2026 games when exact event/team IDs and a pregame capture clock pass validation. The latest validated capture inspected 120 eligible games, found pickcenter data for 49, accepted 145 market rows and rejected 71 incomplete or invalid rows. ESPN supplies no independent quote-update clock, so these rows remain clearly labeled capture-clock evidence. Observations first collected after kickoff cannot be relabeled as historical pregame quotes or closing lines. No market advantage or prospective betting performance is reported. The site still publishes a separate 2025 retrospective benchmark joining 784 exact game IDs to the fixed preseason holdout: it compares model and archived-line margin/total error and winner picks, labels every row as archive-only, and leaves the prospective scorecard unchanged.
...
## Roster and recruiting context

The [football recruiting desk](/football/recruiting/) now reads four additional attributed SportsDataverse release families: ESPN season rosters, recruiting commitments, team talent and returning production. The September 21, 2026 personnel import retains 51,524 season-roster rows, 8,624 recruit rows, 632 team-talent rows and 472 returning-production rows for 2025–26. Rosters carry stable ESPN athlete IDs, team IDs, position, experience, status and listed size where supplied; recruiting rows preserve the source recruit ID, position, stars and grade; team files preserve talent ranks, blue-chip share and returning offensive/defensive production.

`GET /api/football/recruiting?view=rosters&season=2026` returns a bounded, receipt-backed personnel slice. Switch `view` to `recruits`, `talent` or `returning`; all views accept literal `q`, team ID, division, page and limit filters. The recruit view also returns a full-filtered position-room summary with recruit counts, recorded-grade coverage, average recorded grade and source star bands. Position groups must reconcile to the class total before the UI renders them; blank source positions remain an explicit unavailable group. The endpoint keeps raw source fields alongside the display projection, uses a five-second D1 budget and five-minute edge cache, and returns a retryable 503 when the warehouse is unavailable. Personnel records are source context only: they do not establish enrollment, transfer completion, eligibility, depth-chart order or a forecast feature. The raw `/football/source-stats/` browser also exposes these four namespaces for full field inspection.

These releases are processed by the SportsDataverse cfbfastR pipeline and are refreshed on its published schedule; the roster release is annual while game data refreshes during the season. Silvermine stores the release URL, retrieval clock, ETag/last-modified values and SHA-256 receipt, and does not fetch ESPN pages directly. The standalone `scripts/sync-football-recruiting.py` path updates an existing football D1 with only the validated 2025–26 personnel scopes in bounded replayable imports; the regular football publisher includes the same datasets in its complete export.
