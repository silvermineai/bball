...
The player index exposes PPG, RPG, APG, steals, blocks, turnovers, minutes, eFG%, estimated TS% and 3P%. Default qualification is 15 games and 400 recorded minutes, with complete fields. DNP rows remain accessible in game logs. TS uses the same disclosed 0.475 college free-throw coefficient. The national leaders page also exposes eight numeric publisher metrics (including shooting percentages, assist-to-turnover ratio and scoring efficiency) from the attributed 2025–26 season release, with source display values and a 15-game minimum preserved separately from Silvermine-derived rankings. The source-stat browser exposes all 44 retained publisher fields across averages, totals and miscellaneous categories through a paginated D1 API; compound made-attempted fields remain the publisher's display strings, and every row links back to the player source record.
