...
- 120,174 current NCAA-derived player box rows are available through the [NCAA player box archive](/basketball/ncaa-player-box/), retaining game-level counting stats and shooting, transition and playmaking splits. The release is kept in its separate NCAA identity namespace; no name-only join to ESPN records is used.
