...
`espn_pickcenter.py` captures the current `pickcenter` values from ESPN's public game summary endpoint for confirmed 2027 basketball games within a bounded 60-day window (the implementation caps any request at 90 days and 120 events). It requires the summary event ID, competition start instant and home/away ESPN IDs to match the retained schedule exactly, and stores complete two-sided moneyline, spread and total markets only. ESPN does not provide a quote update clock in this response, so the capture clock is recorded as both observation and update time; the connector accepts future games only and never promotes historical summaries into prospective comparisons. Raw responses are kept in the ignored `.local/odds` cache, while the ledger stores a bounded receipt and derived prices. Run `PYTHONPATH=ncaa_scraper .venv/bin/python -m ncaa_scraper.espn_pickcenter --season 2027`, or add `--espn-lines` to `scripts/publish-research.py`. Scheduled refreshes invoke this bounded capture automatically; an offseason run can legitimately record zero quotes when no game in the window has a current pickcenter.
