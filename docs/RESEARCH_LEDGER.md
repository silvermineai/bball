...
`espn_recruiting.py` retrieves the public ESPN men's basketball recruiting list and its linked prospect records for the 2027 class, with four concurrent requests and a two-megabyte response bound. The normalized release retains source rank, grade, position, school references, commitment description, high-school and hometown fields, exact source URL and response hash. The Worker serves `/api/basketball/research/recruiting-rankings` with pagination and search; the recruiting desk labels every value as ESPN source evidence and links to the original prospect card. It is separate from the reviewed school-announcement release and authorized portal intake: a rank or commitment description does not establish a roster spot, transfer date or NCAA eligibility. Raw response bodies remain in the ignored `.local/recruiting/espn` cache, and the scheduled workflow refreshes this release before publication.
