...
Evaluation policy `first-eligible-registration-v1` chooses one forecast per game before examining its result: the earliest eligible local registration. An eligible forecast has confirmed, unchanged participants and start time, with model cutoff ≤ generation ≤ registration < scheduled start. If the schedule changes, the old registration stays excluded; a new eligible model version may qualify later. Missing, inconsistent or unscored finals do not contribute to metrics. Results include overtime. As-of reports ignore observations and registrations from after their cutoff.
