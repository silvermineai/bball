...
The shared Next.js scorecard lives at `/research/scorecard/`. It reads the current edition from the bounded `/api/research/scorecard` endpoint backed by the dedicated Cloudflare D1 research ledger, and falls back to the checked-in edition when the live API is unavailable. Individual game history at `/research/game/?sport=football&id=…` reads Cloudflare D1 through a paginated, read-only API. Basketball uses `sport=basketball`. The scorecard exports both the visible page and every matching filtered game row, including registration clocks, settlement status and serialized qualifying quote evidence. The live endpoint applies the same first-eligible-registration policy and latest-state selection as the local report generator, so the public page can reflect newly settled games and accepted licensed quotes without waiting for a static asset rebuild.

The historical market archive lives at `/research/markets/` and reads retained football archive observations plus any matched basketball/football `audit_markets` quotes through `/api/research/markets`. It is a searchable, paginated research view with page and bounded full-filter CSV exports and a sport selector. Basketball rows expose retained decimal prices and the two-sided implied home probability for moneylines. The archive metadata also lists the connectors available for the selected sport, supported markets and whether a provider-update clock is available. For football, metadata now includes the retained betting-release receipts (release URL, season, fetch clock, SHA-256 and publisher attribution), so the historical line source can be audited without opening a row export. Every row carries its source, observed/capture time and provider update time when supplied; the scorecard only uses observations that pass its provider, capture-time, participant and kickoff checks.
...
`cbbd_lines.py` provides an authorized basketball path through the [CBBD lines endpoint](https://api.collegebasketballdata.com/api/lines), using the server-side `CBBD_API_KEY`. It matches the provider's team names and start time exactly to the retained schedule and records the ingest clock as the capture clock. Because the endpoint does not provide quote update times and its spread/total objects do not include paired prices, the importer stores only valid pregame moneylines as `h2h` observations; spread and total values remain outside the ledger rather than receiving invented prices. Run `PYTHONPATH=ncaa_scraper .venv/bin/python -m ncaa_scraper.cbbd_lines --season 2027`, or add `--cbbd-lines` to `scripts/publish-research.py`. Without a key it fails before making a provider call.

## Public ESPN summary connector

`espn_pickcenter.py` captures the current `pickcenter` values from ESPN's public game summary endpoint for confirmed 2027 basketball games within a bounded 90-day window (the implementation caps any request at 90 days and 120 events). The companion `espn_football_pickcenter.py` applies the same exact-ID and future-game policy to the 2026 college-football slate. Both require the summary event ID, competition start instant and home/away ESPN IDs to match the retained schedule exactly, and store complete two-sided moneyline, spread and total markets only. ESPN does not provide a quote update clock in these responses, so the capture clock is recorded as both observation and update time; the connectors accept future games only and never promote historical summaries into prospective comparisons. Raw responses are kept in the ignored `.local/odds` cache, while the ledger stores bounded receipts and derived prices. Run either module directly, or add `--espn-lines --sport basketball`, `--espn-lines --sport football`, or `--espn-lines --sport both` to `scripts/publish-research.py`; sport-scoped runs only open the matching warehouse. Scheduled refreshes invoke this bounded capture automatically; an offseason run can legitimately record zero basketball quotes when no game in the window has a current pickcenter.

## ESPN prospect release

`espn_recruiting.py` retrieves the public ESPN men's basketball recruiting list and its linked prospect records for the 2025 through 2030 classes, with four concurrent requests and a two-megabyte response bound. Before an edition can replace the current class, the scraper verifies the source count and single-page metadata, rejects invalid or duplicate athlete references, and requires a valid detail response for every listed prospect. A partial refresh fails closed and leaves the prior edition active. The normalized release retains source rank, grade, position, school references, commitment description, high-school and hometown fields, exact source URL and response hash. The Worker serves `/api/basketball/research/recruiting-rankings` with season selection, pagination and search; the recruiting desk labels every value as ESPN source evidence and links to the original prospect card. Destination cards also expose a transparent source-rank points total: each ranked commitment contributes `max(1, 101 − national rank)` and an unranked commitment contributes zero. This is a Silvermine comparison aid, not an official ESPN class ranking. It is separate from the reviewed school-announcement release and authorized portal intake: a rank or commitment description does not establish a roster spot, transfer date or NCAA eligibility. Raw response bodies remain in the ignored `.local/recruiting/espn` cache, and the scheduled workflow refreshes each class before publication.
...
The scorecard surfaces the connector state directly. When no licensed quote has been captured, it shows the bounded server-side setup command and links to this policy instead of presenting an empty comparison table as if the market were checked. A missing key is an unavailable observation, never a zero line or a model edge.
...
Model and market errors are compared on exactly the same settled games, separately by bookmaker and market. A positive spread difference (`model home margin + home spread`) favors the home side; positive total difference favors over. Moneyline probabilities normalize the two inverse decimal prices to remove the two-way overround. For settled spreads and totals, each comparison also reports the quoted side result (home/away or over/under), the model-side result and a paired error tally showing how many scored games favored the model, line or neither; these are hypothetical source-score results, not placed bets, execution prices, official bookmaker settlement or profit estimates.
...
# Validate/build/sync/deploy the current scorecard; scope optional captures when refreshing one sport
.venv/bin/python scripts/publish-research.py --sport basketball
```

Refresh football or basketball first to obtain updated schedules and results. Both existing sport publish scripts now register forecasts and observe source states before rebuilding, then sync append-only ledger rows to D1 before deployment. The repository defines a serialized daily GitHub Actions refresh in [`.github/workflows/refresh-research.yml`](../.github/workflows/refresh-research.yml), with manual sport selection, optional licensed connectors and a bounded public ESPN capture. Manual runs can request `capture_espn_lines`; scheduled runs invoke the ESPN connector automatically and also capture licensed sources when their repository secrets are present. Every sport selection runs its source refresh in batch mode, then performs one combined frontend/Worker validation and deployment so a football-only or basketball-only run cannot silently finish without publishing. The workflow runs a release-health gate for generated timestamps and structural coverage before deployment. Repository secrets are required before its first run. Live account validation, settlement against future real finals and verified closing-line feeds remain open work.

## Release verification

The release passed 27 Python tests (source/model/ledger integrity), five Worker API/routing tests, Worker type checking and the combined Next.js/legacy production build. Browser checks covered desktop/mobile scorecards, sport/status/search filters, live D1 prediction/state histories and archive routing. Settled spread/total/moneyline rendering was checked with isolated browser test data; no fabricated market observations were published. Replaying all 4,646 D1 insert statements wrote zero rows, and sampled original registration timestamps remained unchanged. The first scheduled refresh completed successfully on September 7, 2026, including validation and combined Cloudflare deployment. Live odds collection remains unverified because no provider credential is configured.
