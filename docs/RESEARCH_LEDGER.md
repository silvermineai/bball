...
Refresh football or basketball first to obtain updated schedules and results. Both existing sport publish scripts now register forecasts and observe source states before rebuilding, then sync append-only ledger rows to D1 before deployment. The repository defines a serialized weekly GitHub Actions refresh in [`.github/workflows/refresh-research.yml`](../.github/workflows/refresh-research.yml), with manual sport selection and a licensed odds step. Manual runs still require `capture_odds: true`; scheduled runs capture one snapshot automatically when the repository has supplied `THE_ODDS_API_KEY`, and otherwise publish without odds exactly as before. Every sport selection runs its source refresh in batch mode, then performs one combined frontend/Worker validation and deployment so a football-only or basketball-only run cannot silently finish without publishing. The workflow runs a release-health gate for generated timestamps and structural coverage before deployment. Repository secrets are required before its first run. Live account validation, settlement against future real finals and verified closing-line feeds remain open work.
